...
export type HealthStatus =
  "active" | "stubbed" | "misconfigured" | "disabled" | "quota_limited" | "temporarily_unavailable";
...
async function testFactCheckProviderImpl(): Promise<ProviderTestResult> {
  ...
  if (t.error)
    return baseResult(
      "fact_check",
      cfg.factCheck,
      /quota|rate/i.test(t.error) ? "quota_limited" : "temporarily_unavailable",
      t.ms,
      t.error,
    );
...
async function testTranslationProviderImpl(): Promise<ProviderTestResult> {
  ...
  const t = await timed(async () => {
    ...
    const r = await fetch(url.toString(), {
      ...
      body: new URLSearchParams({ q: "hola mundo" }).toString(),
    });
    if (!r.ok) throw new Error(`HTTP ${r.status}`);
    return r.json();
  });
  ...
  return baseResult(
    ...
    "active",
    t.ms,
    "Language detect returned result",
  );
}
...
async function testGeminiProviderImpl(): Promise<ProviderTestResult> {
  ...
  if (!key)
    return baseResult(
      "gemini_claim_extraction",
      "lovable_ai_gateway",
      "misconfigured",
      null,
      "LOVABLE_API_KEY not present",
    );
  const t = await timed(async () => {
  ...
  if (t.error)
    return baseResult(
      "gemini_claim_extraction",
      "lovable_ai_gateway",
      /quota|rate|429/i.test(t.error) ? "quota_limited" : "temporarily_unavailable",
      t.ms,
      t.error,
    );
  return baseResult(
...
async function testVideoIntelligenceProviderImpl(): Promise<ProviderTestResult> {
  ...
  if (cfg.videoIntelligence === "stub") {
    return baseResult(
      "video_intelligence",
      cfg.videoIntelligence,
      "stubbed",
      null,
      "Requires service account credentials — blocked by GCP org policy",
    );
  }
  return baseResult(
    "video_intelligence",
    ...
    "misconfigured",
    null,
    "Provider mode set but service account activation not implemented in this build",
  );
}
...
export const testAllMultimediaProviders = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const rows = results.map((r) => ({
      ...
      error_message: r.status === "active" ? null : r.message,
      diagnostic: { message: r.message },
      checked_by: userId,
    }));
...
export const testOneProvider = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((raw) =>
    z.object({ provider: z.enum(Object.keys(testers) as [string, ...string[]]) }).parse(raw),
  )
  .handler(async ({ data, context }) => {
    ...
    await supabase.from("provider_health_checks").insert({
      ...
      error_message: result.status === "active" ? null : result.message,
  ...
  .handler(async ({ context }) => {
    ...
    const providers = [
      {
        ...
        credential: cfg.factCheckApiKey ? "configured" : "missing",
      },
      {
        key: "translation",
        label: "Google Translation",
        mode: cfg.translation,
        flag: "MM_PROVIDER_TRANSLATION",
        credential: cfg.googleApiKey ? "configured" : "missing",
      },
      {
        key: "gemini_claim_extraction",
        label: "Gemini Claim Extraction",
        mode: "lovable_ai_gateway",
        flag: "LOVABLE_API_KEY",
        credential: process.env.LOVABLE_API_KEY ? "configured" : "missing",
      },
      {
        key: "video_intelligence",
        label: "Video Intelligence",
        mode: cfg.videoIntelligence,
        flag: "MM_PROVIDER_VIDEO_INTELLIGENCE",
        credential: cfg.hasServiceAccount ? "configured" : "missing",
      },
      {
        key: "speech_to_text",
        label: "Speech-to-Text",
        mode: cfg.speechToText,
      ...
      },
      {
        ...
        label: "Cloud Storage",
        mode: "lovable_cloud",
        flag: "GOOGLE_CLOUD_STORAGE_BUCKET",
        credential: cfg.bucket ? "configured" : "missing",
      },
    ];
    ...
    for (const p of providers) {
      ...
      stats[p.key] = {
        ...
        currentAvailability:
          lastOk && (!lastFail || new Date(lastOk.created_at) > new Date(lastFail.created_at))
            ? "up"
            : total
              ? "down"
              : "unknown",
    ...
    const { data: usage } = await supabase
      .from("quota_usage")
      .select("analyses_count, cost_cents, api_calls_count")
      .eq("usage_date", today);
