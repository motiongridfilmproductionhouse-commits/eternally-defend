...
export async function checkAndReserveQuota(
  ...
  const [todayRow, monthAgg] = await Promise.all([
    supabase
      .from("quota_usage")
      .select("analyses_count, cost_cents")
      .eq("user_id", userId)
      .eq("usage_date", today)
      .maybeSingle(),
    supabase
  ...
  const monthCostCents = (monthAgg.data ?? []).reduce(
    (s: number, r: any) => s + (r.cost_cents ?? 0),
    0,
  );
  ...
  if (todayUsed >= limits.dailyAnalysisLimit) {
    return {
      allowed: false,
      reason: `Daily analysis limit reached (${limits.dailyAnalysisLimit}/day)`,
      todayUsed,
      monthCostCents,
      limits,
    };
  ...
  if ((monthCostCents + costCents) / 100 > limits.monthlyCostLimitUsd) {
    return {
      allowed: false,
      reason: `Monthly cost limit would be exceeded ($${limits.monthlyCostLimitUsd})`,
      todayUsed,
      monthCostCents,
      limits,
    };
  ...
  return {
    ...
    monthCostCents: monthCostCents + costCents,
    limits,
  };
}
