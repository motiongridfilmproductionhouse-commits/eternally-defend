...
export function explainRiskScores(i: RiskInputs): RiskExplanation {
  ...
  const factCheckSig = {
    signal: "fact_checks_false",
    label: "Fact checks rated false / misleading",
    severity: 8,
    count: i.factChecksFalse,
    confidence: 0.9,
    weight: 1,
  };
  const factCheckReviewed = {
    signal: "fact_checks_reviewed",
    label: "Fact-check reviews found (any rating)",
    severity: 2,
    count: i.factChecksReviewed,
    confidence: 0.75,
    weight: 1,
  };
  const criticalFinding = {
    signal: "critical_findings",
    label: "Critical timeline findings",
    severity: 20,
    count: i.criticalFindings,
    confidence: 0.95,
    weight: 1,
  };
  const highFinding = {
    signal: "high_findings",
    label: "High-severity findings",
    severity: 10,
    count: i.highFindings,
    confidence: 0.9,
    weight: 1,
  };
  const mention = {
    signal: "transcript_hits",
    label: "Protected name mentions in text/transcript",
    severity: 3,
    count: i.transcriptHits,
    confidence: evConf,
    weight: entityRel,
  };
  const visual = {
    signal: "visual_hits",
    label: "Visual/logo/photo matches",
    severity: 6,
    count: i.visualHits,
    confidence: evConf,
    weight: entityRel,
  ...
  const asset = {
    signal: "asset_matches",
    label: "Protected asset matches (>0.8 similarity)",
    severity: 12,
    count: i.assetMatches,
    confidence: 0.95,
    weight: 1,
  };
  const reachSig = {
    signal: "reach",
    label: `Audience reach — ${reach.tier}`,
    severity: reach.pts,
    count: 1,
    confidence: 0.8,
    weight: 1,
  ...
  const explanations = {
    ...
    misinformation: axis(
      [
        { ...factCheckSig, severity: 22 },
        { ...factCheckReviewed, severity: 6 },
      ],
      missingFor(["fact_checks_false", "fact_checks_reviewed"], i),
    ),
    harassment: axis(
      [
        { ...highFinding, severity: 8 },
        { ...criticalFinding, severity: 15 },
      ],
    ...
    evidenceConfidence: axis(
      [
        {
          signal: "transcript_confidence",
          label: "Transcript avg confidence",
        ...
        {
          signal: "fact_check_present",
          label: "Fact-check evidence available",
          severity: i.factChecksReviewed ? 20 : 0,
          count: 1,
          confidence: 1,
          weight: 1,
        },
        {
          signal: "translation_ok",
          label: "Translation confidence adequate",
          severity: i.translationLowConfidence ? 0 : 20,
          count: 1,
          confidence: 1,
          weight: 1,
        },
