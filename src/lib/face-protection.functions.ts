...
async function ensureCollectionForUser(
  userId: string,
  supabase: {
    from: (t: string) => {
      select: (c: string) => {
...
export const importOfficialAccountFaces = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    for (const url of urls) {
      ...
      if (!img) {
        errors.push(`skip ${url} (unreachable / not an image)`);
        continue;
      }
      ...
      try {
        await putObject({
          ...
          metadata: { source: url.slice(0, 512) },
        });
...
export const listProtectedFaces = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabase
      .from("protected_faces")
      .select(
        "id,platform,label,source_url,s3_key,face_id,confidence,created_at,asset_id,discovered_account_id",
      )
      .eq("user_id", userId)
...
export const deleteProtectedFace = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      /* ignore Rekognition error, still remove DB row */
    }
