...
export function getIntegrationDiagnostics(): IntegrationDiagnostics {
  ...
  const supabase =
    hasSupabaseUrl && hasSupabaseService
      ? "configured"
      : hasSupabaseUrl || hasSupabaseService
  ...
  const awsCount = [hasAwsRegion, hasAwsAccessKey, hasAwsSecretKey, hasAwsBucket].filter(
    Boolean,
  ).length;
  const aws =
    awsCount === 4 ? "configured" : awsCount > 0 ? "partially configured" : "not configured";
  ...
  const veriff =
    veriffCount === 2 ? "configured" : veriffCount > 0 ? "partially configured" : "not configured";
  ...
  const gcpCount = [hasGcpProjectId, hasGcpBucket, hasGcpCreds, hasGoogleApiKey].filter(
    Boolean,
  ).length;
  const googleCloud =
    gcpCount >= 2 ? "configured" : gcpCount > 0 ? "partially configured" : "not configured";
