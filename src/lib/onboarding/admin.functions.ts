...
export const decideAuthorization = createServerFn({ method: "POST" })
  ...
  .inputValidator(
    (d: {
      ...
      decision: "approve" | "reject" | "request_info" | "suspend" | "revoke" | "renew";
      notes?: string;
    }) =>
      z
        .object({
  ...
  .handler(async ({ data, context }) => {
    ...
    const nextStatus: Record<string, string> = {
      approve: "ACTIVE",
      reject: "REJECTED",
      request_info: "AWAITING_SIGNATURE",
      suspend: "SUSPENDED",
      revoke: "REVOKED",
      renew: "ACTIVE",
    };
    ...
    if (data.decision === "approve" || data.decision === "renew") {
      ...
      const [{ data: profile }, { data: kyc }, { data: face }, { data: assets }, { data: sigs }] =
        await Promise.all([
          supabase.from("client_profiles").select("*").eq("user_id", auth.user_id).maybeSingle(),
          supabase
            .from("kyc_verifications")
      ...
      line(
        `Name: ${profile?.display_name ?? (profile as { full_name?: string } | null)?.full_name ?? ""}`,
      );
      ...
      try {
        ...
        drawUnicodeText(page, `Verify: /verify/${public_slug}`, {
          x: 380,
          y: 78,
          size: 8,
          stack: stack.regular,
      ...
      await supabase
        .from("client_authorizations")
        .update({ status: newStatus as never, enforcement_enabled: false })
        .eq("id", auth.id);
    }
