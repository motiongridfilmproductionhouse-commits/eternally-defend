...
export const listAssets = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    return (
      (await supabase.from("digital_assets").select("*").eq("user_id", userId).order("created_at"))
        .data ?? []
    );
  });
...
export const addYouTubeAsset = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { channel_url: string }) =>
    z.object({ channel_url: z.string().min(3).max(300) }).parse(d),
  )
...
export const generateChallenge = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await supabase
      .from("youtube_verification_challenges")
      .insert({
        user_id: userId,
    ...
    await supabase
      .from("digital_assets")
      .update({ verification_status: "CODE_GENERATED" })
      .eq("id", data.asset_id)
      .eq("user_id", userId);
    return row;
  });
...
export const verifyChallenge = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const chRes = await fetch(
      `https://www.googleapis.com/youtube/v3/channels?part=snippet,brandingSettings&id=${asset.channel_id}&key=${apiKey}`,
    );
    ...
    const chDesc =
      (chJ.items?.[0]?.snippet?.description ?? "") +
      "\n" +
      (chJ.items?.[0]?.brandingSettings?.channel?.description ?? "");

    let found = chDesc.includes(chal.code);
    let evidence: { source: string; matched: boolean; video_id?: string } = {
    ...
    if (!found) {
      const sr = await fetch(
        `https://www.googleapis.com/youtube/v3/search?part=id&channelId=${asset.channel_id}&order=date&maxResults=5&type=video&key=${apiKey}`,
      );
    ...
    await supabase
      .from("youtube_verification_challenges")
      .update({ used_at: new Date().toISOString(), evidence: evidence as never })
      .eq("id", chal.id);
    await supabase
      .from("digital_assets")
      .update({
        verification_status: "VERIFIED",
        verification_method: "code_challenge",
