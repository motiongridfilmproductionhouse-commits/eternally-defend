...
async function assertOwnedProfile(
  ...
  if (error || !data) {
    throw new Error(
      "Target profile was not found or is not accessible.",
    );
  }
...
export const createDeepfakeTargetProfile =
  createServerFn({ method: "POST" })
    .middleware([requireSupabaseAuth])
    .inputValidator((input: unknown) =>
      CreateProfileInput.parse(input),
    )
    .handler(async ({ data, context }) => {
      ...
      if (error || !profile) {
        throw new Error(
          error?.message ??
            "Failed to create target profile.",
        );
      }
...
export const listDeepfakeTargetProfiles =
...
export const getDeepfakeTargetProfile =
  createServerFn({ method: "GET" })
    .middleware([requireSupabaseAuth])
    .inputValidator((input: unknown) =>
...
export const uploadDeepfakeReferenceFace =
  createServerFn({ method: "POST" })
    .middleware([requireSupabaseAuth])
    .inputValidator((input: unknown) =>
      UploadReferenceInput.parse(input),
    )
    .handler(async ({ data, context }) => {
      const { supabase, userId } = context;
      ...
      const imageBytes = decodeBase64Image(
        data.image_base64,
      );
      ...
      if (uploadError) {
        throw new Error(
          `Reference image upload failed: ${uploadError.message}`,
        );
      }

      try {
        const indexed =
          await indexDeepfakeReferenceFace({
            imageBytes,
...
export const deleteDeepfakeReferenceFace =
  createServerFn({ method: "POST" })
    .middleware([requireSupabaseAuth])
    .inputValidator((input: unknown) =>
      DeleteReferenceInput.parse(input),
    )
    .handler(async ({ data, context }) => {
      const { supabase, userId } = context;
      ...
      if (reference.rekognition_face_id) {
        try {
          ...
          console.warn(
            "[DEEPFAKE:FACE] Failed to delete Rekognition face:",
            awsError,
          );
        }
      }
