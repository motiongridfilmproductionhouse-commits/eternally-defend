...
export async function runEipTick(
  engine: EnginePort,
  db: DbPort,
  cfg: WorkerConfig,
): Promise<TickResult> {
  ...
  try {
    ...
    health = parsed.data;
    if (!health.engineAvailable || !health.modelsLoaded)
      throw new EngineError(
        health.modelsLoaded ? "ENGINE_UNAVAILABLE" : "MODEL_LOAD_FAILED",
        "engine not ready",
      );
    if (health.configIntegrity === "failed")
      throw new EngineError("CONFIG_INTEGRITY_FAILED", "engine reports config integrity failure");
    const v = checkVersions(
      { engineVersion: health.engineVersion, configSha256: health.configSha256 },
      cfg.expected,
    );
    ...
    healthState =
      healthCode === "ENGINE_VERSION_MISMATCH"
        ? "VERSION_MISMATCH"
        : healthCode === "CONFIG_INTEGRITY_FAILED"
  ...
  for (const r of await db.claimReevals(cfg.batch)) {
    ...
    try {
      ...
      const code: EipErrorCode = e instanceof EngineError ? e.code : "ENGINE_UNAVAILABLE";
      await db.report(null, {
        reevaluation_id: r.id,
        job_id: r.job_id,
        error_code: code,
        detail: sanitize(e),
      });
      if (code !== "ENGINE_UNAVAILABLE" && code !== "ENGINE_TIMEOUT")
        await db.updateReeval(r.id, "SYSTEM_ERROR", null, code, null);
    }
  }
...
async function advanceJob(
  ...
  if (now() - started > cfg.jobTimeoutSeconds * 1000)
    throw new EngineError("ENGINE_TIMEOUT", "job exceeded timeout");
  ...
  if (!job.engine_job_id) {
    ...
    if (res.data.platformJobId !== job.id)
      throw new EngineError("ENGINE_RESPONSE_INVALID", "platformJobId mismatch");
  ...
  if (s.status === "FAILED_SYSTEM") {
    ...
    const code = reasons.includes("AUTHORIZATION_INVALID")
      ? "AUTHORIZATION_INVALID"
      : (s.errorCode ?? "ENGINE_UNAVAILABLE");
    await db.report(null, {
      job_id: job.id,
      error_code: code,
      detail: "engine reported system failure",
    });
  ...
  const check = validateManifest(
    s.manifest,
    job,
    { engineVersion: job.engine_version, configSha256: job.config_sha256 },
    cfg.expected,
  );
  ...
  if (m.protected_asset_sha256) {
    ...
    if ((await sha256Hex(bytes)) !== m.protected_asset_sha256)
      throw new EngineError("OUTPUT_HASH_MISMATCH", "protected hash mismatch");
    try {
  ...
  await db.updateJob(
    job.id,
    {
      ...
      certificate_id:
        decision === "FAIL"
          ? null
          : (m.certificate_id ?? `EIP-${job.id.slice(0, 8).toUpperCase()}`),
...
async function advanceReeval(r: Reeval, engine: EnginePort, db: DbPort, cfg: WorkerConfig) {
  if (!r.protected_storage_path || !r.protected_sha256)
    throw new EngineError("EVALUATION_INCOMPLETE", "no protected asset");
  if (!r.engine_evaluation_id) {
    ...
    const res = (await engine.submitEvaluation(
      {
        platformJobId: r.job_id,
        reevaluationId: r.id,
        protectedUrl: url,
        protectedSha256: r.protected_sha256,
        evaluationVersion: cfg.evaluationVersion ?? null,
      },
      r.id,
    )) as { engineEvaluationId?: string };
    if (!res?.engineEvaluationId)
      throw new EngineError("ENGINE_RESPONSE_INVALID", "evaluation submit");
  ...
  if (st.data.status === "FAILED_SYSTEM" || !st.data.evaluation)
    throw new EngineError("EVALUATION_INCOMPLETE", st.data.errorCode ?? "no evaluation");
  const ev = st.data.evaluation;
  if (ev.protected_asset_sha256 !== r.protected_sha256)
    throw new EngineError("OUTPUT_HASH_MISMATCH", "evaluated asset hash mismatch");
