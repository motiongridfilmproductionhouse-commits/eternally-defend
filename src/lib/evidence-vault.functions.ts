...
const UploadInput = z.object({
  ...
  base64: z
    .string()
    .min(1)
    .max(20 * 1024 * 1024), // ~15MB decoded ceiling
...
export const uploadEvidence = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await supabase
      ...
      .select("id,s3_key,sha256,bytes,created_at")
      .single();
...
export const listEvidence = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    let q = supabase
      .from("evidence_vault_items")
      .select(
        "id,kind,label,s3_key,sha256,bytes,content_type,created_at,case_id,enforcement_request_id,scan_hit_id,face_match_event_id",
      )
      .eq("user_id", userId)
