...
function paragraph(doc: PDFDocument, c: Cursor, text: string, size = 10.5): Cursor {
  ...
  for (const w of words) {
    ...
    if (width > maxWidth && line) {
      ...
      drawUnicodeText(c.page, line, {
        x: 60,
        y: c.y,
        size,
        stack: c.stack.regular,
...
export async function generatePartnerMou(input: MouInput): Promise<Uint8Array> {
  ...
  drawUnicodeText(
    c.page,
    `Agreement No: ${input.agreementNumber}   |   Effective: ${input.effectiveDate}`,
    {
      x: 60,
      y: c.y,
      size: 9.5,
  ...
  c = bullet(
    doc,
    c,
    "Do not incentivise sign-ups with cashbacks, kickbacks or payments to prospects.",
  );
  ...
  c = bullet(
    doc,
    c,
    "Report commissions accurately and settle payable commissions in the agreed cycle.",
  );
  ...
  c.y -= 10;
  drawUnicodeText(c.page, "Signed for and on behalf of:", {
    x: 60,
    y: c.y,
    size: 10.5,
    stack: stack.bold,
  ...
  const colY = c.y;
  // Eterna column
  drawUnicodeText(c.page, ETERNA_NAME, {
    x: 60,
    y: colY,
    size: 10.5,
    stack: stack.bold,
  ...
  drawUnicodeText(
    c.page,
    `Signatory: ${input.eternaSignerName ?? "____________________________"}`,
    {
      x: 60,
      y: colY - 18,
      size: 10,
  ...
  drawUnicodeText(
    c.page,
    `Date: ${input.eternaSignedAt ? input.eternaSignedAt.slice(0, 10) : "____________"}`,
    {
      x: 60,
      y: colY - 34,
      size: 10,
  ...
  drawUnicodeText(
    c.page,
    "Status: " + (input.eternaSignedAt ? "SIGNED" : "AWAITING ETERNA APPROVAL"),
    {
      x: 60,
      y: colY - 50,
      size: 9.5,
  ...
  // Partner column
  drawUnicodeText(c.page, input.partnerCompany, {
    x: 320,
    y: colY,
    size: 10.5,
    stack: stack.bold,
  ...
  drawUnicodeText(
    c.page,
    `Date: ${input.partnerSignedAt ? input.partnerSignedAt.slice(0, 10) : input.effectiveDate}`,
    {
      x: 320,
      y: colY - 34,
      size: 10,
