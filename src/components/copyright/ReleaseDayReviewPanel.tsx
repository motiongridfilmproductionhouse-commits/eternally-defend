...
import {
  runReleaseDayReviewAnalysis, listReleaseDayReviews, updateYoutubeMonitorReview,
} from "@/lib/copyright/youtube-monitor.functions";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { toast } from "sonner";
import {
  Loader2, CalendarClock, ExternalLink, Eye, XCircle, AlertTriangle,
  ThumbsDown, ThumbsUp, Minus, Clock, MessageSquare,
} from "lucide-react";
...
interface KeyStatement { statement: string; kind: string; timestamp?: string | null }
    ...
    <section className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border/60 bg-card/60 p-4 backdrop-blur">
        <div className="min-w-0">
          <h3 className="flex items-center gap-2 text-sm font-semibold">
            <CalendarClock className="h-4 w-4 text-primary" />Release Day Review &amp; Reputation Analysis
          </h3>
          <p className="mt-1 max-w-3xl text-xs text-muted-foreground">
            Monitors reviews and reaction videos published around the release window — same-day reviews, first reactions,
            early access, critics and regional-language coverage — for misleading claims, false statements and harmful
            narratives. Monitoring insight and evidence only; nothing is reported, removed or sent to creators.
          </p>
          {rows.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-2 text-[11px]">
              <Badge variant="outline" className={IMPACT.high.cls}>{counts.high} high</Badge>
              <Badge variant="outline" className={IMPACT.medium.cls}>{counts.medium} medium</Badge>
              <Badge variant="outline" className={IMPACT.low.cls}>{counts.low} low</Badge>
            </div>
          )}
        </div>
        <Button size="sm" onClick={() => run.mutate()} disabled={run.isPending}>
          {run.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <CalendarClock className="mr-2 h-4 w-4" />}
      ...
      {!reviews.isLoading && !rows.length && (
        <div className="rounded-lg border border-border/60 bg-card/50 p-6 text-sm text-muted-foreground">
          No release-window reviews analysed yet. Run the analysis to collect reputation intelligence.
        </div>
      )}

      {rows.map((v) => {
        const impact = IMPACT[v.reputation_impact] ?? IMPACT.low;
        const statements = (Array.isArray(v.key_statements) ? v.key_statements : []) as unknown as KeyStatement[];
        const signals = (Array.isArray(v.misleading_signals) ? v.misleading_signals : []) as unknown as string[];
        const stamps = (Array.isArray(v.evidence_timestamps) ? v.evidence_timestamps : []) as unknown as string[];
        const comments = (Array.isArray(v.comment_samples) ? v.comment_samples : []) as unknown as string[];
        return (
          <article key={v.id} className="rounded-xl border border-border/60 bg-card/60 p-4 backdrop-blur">
            <div className="flex gap-4">
              {v.thumbnail_url && (
                <img src={v.thumbnail_url} alt={`Thumbnail of release-window review video ${v.title}`} loading="lazy"
                  className="h-24 w-40 shrink-0 rounded-lg border border-border/60 object-cover" />
              )}
              <div className="min-w-0 flex-1 space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  <Badge variant="outline" className={impact.cls}>{impact.label} · {v.reputation_impact_score}</Badge>
                  <Badge variant="outline" className="text-[10px] capitalize">
                    {(v.review_type ?? "review").replace(/_/g, " ")}
                  </Badge>
                  <Badge variant="outline" className="flex items-center gap-1 text-[10px] capitalize">
                    <SentimentIcon s={v.sentiment} />{v.sentiment} {v.sentiment_score ?? 0}
                  </Badge>
                ...
                {signals.length > 0 && (
                  <div className="flex flex-wrap items-center gap-1">
                    ...
                    {signals.map((s) => (
                      <span key={s} className="rounded border border-red-600/40 px-1.5 py-0.5 text-[10px] text-red-400">
                        {s.replace(/_/g, " ")}
                      </span>
                    ))}
                  </div>
                ...
                {statements.length > 0 && (
                  <ul className="space-y-1">
                    {statements.map((s, i) => (
                      <li key={`${i}-${s.statement.slice(0, 12)}`}
                        className={`rounded border px-2 py-1 text-[11px] ${STATEMENT_CLS[s.kind] ?? STATEMENT_CLS.opinion}`}>
                        <span className="font-medium uppercase tracking-wide">{s.kind.replace(/_/g, " ")}</span>
                        {s.timestamp ? <span className="ml-1 opacity-70">@{s.timestamp}</span> : null}
                ...
                {comments.length > 0 && (
                  <details className="text-[11px] text-muted-foreground">
                    ...
                    <ul className="mt-1 space-y-1 pl-4">
                      {comments.map((c, i) => <li key={i} className="list-disc">{c}</li>)}
                    </ul>
                  </details>
                )}
                ...
                <div className="flex gap-2 pt-1">
                  <Button size="sm" variant="outline"
                    onClick={() => mark.mutate({ videoRowId: v.id, reviewStatus: "evidence_ready" })}>
