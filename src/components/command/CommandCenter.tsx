...
import { FaceReferenceCoveragePanel } from "@/components/command/FaceReferenceCoveragePanel";
import {
  ScanFace,
  UserX,
  Award,
...
const ZONE_STYLE: Record<string, { label: string; color: string; ring: string }> = {
  SAFE: {
    ...
    ring: "shadow-[0_0_60px_-15px_oklch(0.7_0.16_155_/_0.6)]",
  },
  WATCH: {
    label: "WATCH",
    color: "oklch(0.78 0.15 85)",
    ring: "shadow-[0_0_60px_-15px_oklch(0.78_0.15_85_/_0.6)]",
  },
  DANGER: {
    label: "DANGER",
    color: "oklch(0.72 0.18 55)",
    ring: "shadow-[0_0_60px_-15px_oklch(0.72_0.18_55_/_0.6)]",
  },
  CRITICAL: {
    label: "CRITICAL",
    color: "oklch(0.63 0.24 25)",
    ring: "shadow-[0_0_80px_-15px_oklch(0.63_0.24_25_/_0.7)] animate-pulse",
  },
};
    ...
    return (
      <div className="min-h-[60vh] grid place-items-center text-muted-foreground">
        <div className="flex items-center gap-3">
          <Loader2 className="size-4 animate-spin" /> Booting Command Center…
        </div>
      </div>
    );
  }
  if (q.error) {
    return (
      <div className="p-6 text-danger">
        Failed to load intelligence: {(q.error as Error).message}
      </div>
    );
  }
...
function Glass({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <div
      className={`relative rounded-2xl border border-white/10 bg-background/60 backdrop-blur-md shadow-[0_10px_40px_-15px_oklch(0.2_0.1_260_/_0.4)] ${className}`}
    >
      {children}
    </div>
  );
...
function TopIntelBar({ d }: { d: CmdData }) {
  const items = [
    ...
    {
      label: "Threat Level",
      value: d.top.threatLevel,
      tone:
        SEV_COLOR[
          d.top.threatLevel === "Safe"
            ? "Low"
            : d.top.threatLevel === "Moderate"
              ? "Medium"
              : d.top.threatLevel
        ] ?? SEV_COLOR.Low,
      icon: ShieldAlert,
    ...
    {
      label: "Protection",
      value: (d.protection.level ?? "monitoring").replace(/_/g, " "),
      tone: "oklch(0.62 0.19 256)",
      icon: ShieldCheck,
    },
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-8 gap-3">
        {items.map((it) => {
          ...
          return (
            <div
              key={it.label}
              className="flex items-center gap-3 px-3 py-2 rounded-xl border border-white/5 bg-white/[0.02]"
            >
              <div
                className="size-9 rounded-lg grid place-items-center shrink-0"
                style={{
                  background: `color-mix(in oklab, ${it.tone} 16%, transparent)`,
                  color: it.tone,
                }}
              >
              ...
              <div className="min-w-0">
                <div className="text-[9px] tracking-[0.18em] font-semibold uppercase text-muted-foreground truncate">
                  {it.label}
                </div>
                <div
                  className="text-sm font-bold font-display tabular-nums truncate"
                  style={{ color: it.tone }}
                >
                  {String(it.value)}
                </div>
  ...
  return (
    <Glass className="p-5 h-full">
      <CardHeader
        ...
        sub={`${d.radar.length} live threat nodes across ${platforms.length} platforms`}
      />
        ...
        <svg viewBox={`0 0 ${size} ${size}`} width="100%" style={{ maxWidth: size }}>
          {[0.9, 0.65, 0.4, 0.18].map((f, i) => (
            <circle
              ...
              fill={
                i === 0
                  ? "oklch(0.7 0.16 155 / 0.06)"
                  : i === 1
                    ? "oklch(0.78 0.15 85 / 0.06)"
                    : i === 2
                      ? "oklch(0.72 0.18 55 / 0.06)"
                      : "oklch(0.63 0.24 25 / 0.08)"
              }
          ...
          <text
            ...
            y={cy - rMax * 0.94}
            textAnchor="middle"
            className="fill-[oklch(0.7_0.16_155)]"
            fontSize="9"
            fontWeight="700"
            letterSpacing="2"
          >
          ...
          <text
            ...
            y={cy - rMax * 0.69}
            textAnchor="middle"
            className="fill-[oklch(0.78_0.15_85)]"
            fontSize="9"
            fontWeight="700"
            letterSpacing="2"
          >
          ...
          <text
            ...
            y={cy - rMax * 0.44}
            textAnchor="middle"
            className="fill-[oklch(0.72_0.18_55)]"
            fontSize="9"
            fontWeight="700"
            letterSpacing="2"
          >
          ...
          <text
            ...
            y={cy - rMax * 0.13}
            textAnchor="middle"
            className="fill-[oklch(0.63_0.24_25)]"
            fontSize="9"
            fontWeight="700"
            letterSpacing="2"
          >
          ...
          {platforms.map((p, i) => {
            ...
            return (
              <line
                ...
                x2={x}
                y2={y}
                stroke="oklch(0.65 0.18 240 / 0.15)"
                strokeWidth={1}
              />
            );
          })}
          ...
          {d.radar.map((n, idx) => {
            ...
            const a =
              (platformIdx / Math.max(platforms.length, 1)) * Math.PI * 2 -
              Math.PI / 2 +
              jitter * 0.3;
        ...
        {hover && (
          <div className="absolute top-2 left-2 max-w-[260px] rounded-lg border border-white/10 bg-background/95 backdrop-blur px-3 py-2 text-xs shadow-lg pointer-events-none">
            ...
            <div className="text-muted-foreground text-[10px] mt-0.5">
              {hover.platform} · {hover.severity} · Threat {Math.round(hover.threatScore)} · Reach{" "}
              {fmt(hover.reach)}
            </div>
          </div>
        )}
      ...
      <div className="flex items-center justify-center gap-4 mt-3 text-[10px] text-muted-foreground">
        {(["Critical", "High", "Medium", "Low"] as const).map((s) => (
          <span key={s} className="flex items-center gap-1.5">
            <span className="size-2 rounded-full" style={{ background: SEV_COLOR[s] }} />
            {s}
          </span>
        ))}
      </div>
      ...
      <div className="flex items-center gap-5">
        <div className="relative size-[130px] shrink-0">
          ...
          <div className="absolute inset-0 grid place-items-center text-center">
            <div>
              ...
              <div className="text-[9px] tracking-[0.2em] font-semibold text-muted-foreground">
                /100
              </div>
            </div>
          </div>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-2 mt-2 text-xs">
            ...
            <span>
              {d.danger.velocityDelta > 0 ? "+" : ""}
              {d.danger.velocityDelta} findings vs prior 24h
            </span>
          </div>
        </div>
      </div>
...
function ThreatIntelOverview({ d }: { d: CmdData }) {
  const kpis = [
    {
      label: "Total",
      value: d.overview.totalFindings,
      trend: d.overview.findingsTrend,
      spark: d.overview.findingsSpark,
      tone: "oklch(0.65 0.18 240)",
    },
    {
      label: "Critical",
      value: d.overview.criticalFindings,
      trend: d.overview.criticalTrend,
      spark: d.overview.criticalSpark,
      tone: "oklch(0.63 0.24 25)",
    },
    {
      label: "New Today",
      value: d.overview.newToday,
      trend: "up" as const,
      spark: d.overview.findingsSpark,
    ...
    {
      label: "Resolved",
      value: d.overview.resolved,
      trend: "up" as const,
      spark: d.overview.findingsSpark,
      tone: "oklch(0.7 0.16 155)",
    },
    {
      label: "False +",
      value: d.overview.falsePositives,
      trend: "flat" as const,
      spark: d.overview.findingsSpark,
      tone: "oklch(0.68 0.09 240)",
    },
  ];
  return (
    <Glass className="p-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        {kpis.map((k) => (
          <div key={k.label} className="rounded-xl border border-white/5 bg-white/[0.02] p-3">
            <div className="text-[9px] tracking-widest uppercase text-muted-foreground font-semibold">
              {k.label}
            </div>
      ...
      <div className="space-y-3 text-sm">
        ...
        <div>
          ...
          <ul className="space-y-1 text-xs text-foreground/80">
            <li>
              · {d.overview.criticalFindings} critical, {d.overview.totalFindings} total findings
              last 14d
            </li>
            <li>
              · {d.overview.newToday} new in the last 24h ({d.danger.velocityDelta >= 0 ? "+" : ""}
              {d.danger.velocityDelta} vs prior)
            </li>
        ...
        {topThreat && (
          <div>
            ...
            <div className="text-[10px] text-muted-foreground mt-0.5">
              {topThreat.platform} · Threat {Math.round(topThreat.threatScore)}
            </div>
          </div>
        )}
        <div>
          ...
          <ul className="space-y-1 text-xs text-foreground/80">
            {d.overview.criticalFindings > 0 && (
              <li>· Review {d.overview.criticalFindings} critical findings</li>
            )}
            {d.top.pendingActions > 0 && (
              <li>· Advance {d.top.pendingActions} pending enforcement actions</li>
            )}
            {d.top.protectedAssets === 0 && <li>· Register at least one protected asset</li>}
            {d.overview.criticalFindings === 0 && d.top.pendingActions === 0 && (
              <li>· All clear — continue monitoring</li>
            )}
          </ul>
        </div>
      </div>
...
function LiveScannerPanel({ d }: { d: CmdData }) {
  ...
  return (
    <Glass className="p-5">
      <CardHeader
        label="Live Scanner Panel"
        sub="Reconnaissance surface status"
        action={
          <Link
            to="/scan"
            className="text-[11px] text-primary hover:underline inline-flex items-center gap-1"
          >
            Run scan <ChevronRight className="size-3" />
          </Link>
        }
      />
            ...
            <div key={s.kind} className="rounded-xl border border-white/5 bg-white/[0.02] p-3">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {isRunning && (
                      <Loader2 className="size-3 animate-spin" style={{ color: tone }} />
                    )}
                  </div>
                  <div className="text-[11px] text-muted-foreground truncate">
                    {s.query ?? "No active query"}
                  </div>
                </div>
              ...
              <div className="h-1 mt-2 rounded-full bg-white/5 overflow-hidden">
                <div
                  ...
                  style={{ width: `${s.progress}%`, background: tone }}
                />
              </div>
            </div>
          );
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="text-muted-foreground">
                ...
                {sevs.map((s) => (
                  <th key={s} className="pb-2 px-2 font-semibold" style={{ color: SEV_COLOR[s] }}>
                    {s}
                  </th>
                ))}
                  ...
                  {sevs.map((s) => {
                    ...
                    return (
                      <td key={s} className="px-1 py-1">
                        <div
                          className="rounded-md text-center py-1.5 font-semibold tabular-nums text-[11px]"
                          style={{
                            background: `color-mix(in oklab, ${SEV_COLOR[s]} ${Math.round(8 + intensity * 60)}%, transparent)`,
                            color: intensity > 0.3 ? "white" : SEV_COLOR[s],
                          }}
                        >
                          {v || ""}
                        </div>
...
function ReputationSpoilerDetector({ d }: { d: CmdData }) {
  return (
    <Glass className="p-5">
      <CardHeader
        label="Reputation Spoiler Detector"
        sub="AI-classified content likely to damage reputation"
      />
      {d.spoilers.length === 0 ? (
        <div className="text-sm text-muted-foreground py-6 text-center">
          No reputation spoilers detected in the last 14 days.
        </div>
      ) : (
  ...
  return (
    <Glass className="p-5">
      <CardHeader
        ...
        sub="Top 10 by threat × reach"
        action={
          <Link
            to="/threat-radar"
            className="text-[11px] text-primary hover:underline inline-flex items-center gap-1"
          >
            Open radar <ChevronRight className="size-3" />
          </Link>
        }
      />
      {d.trending.length === 0 ? (
        <div className="text-sm text-muted-foreground py-6 text-center">
          Nothing trending — you're clear.
        </div>
      ) : (
        <div className="space-y-2">
          {d.trending.map((t, i) => {
            ...
            return (
              <div
                key={t.id}
                className="flex items-center gap-3 p-2.5 rounded-xl border border-white/5 bg-white/[0.02] hover:bg-white/[0.05] transition"
              >
                <div className="text-xs font-bold tabular-nums w-5 text-muted-foreground">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <div className="text-right w-10">
                    <div className="text-sm font-bold tabular-nums" style={{ color: tone }}>
                      {Math.round(t.threatScore)}
                    </div>
                  </div>
                  {t.permalink && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="size-7 grid place-items-center rounded-lg border border-white/10 hover:border-primary/40 text-muted-foreground hover:text-primary"
                    >
                  ...
                  <Link
                    to="/scan"
                    className="size-7 grid place-items-center rounded-lg border border-white/10 hover:border-primary/40 text-muted-foreground hover:text-primary"
                    title="View / evidence / take action"
                  >
  ...
  return (
    <Glass className="p-5 h-full">
      <CardHeader
        ...
        sub="Most targeted protected assets"
        action={
          <Link
            to="/assets"
            className="text-[11px] text-primary hover:underline inline-flex items-center gap-1"
          >
            Assets <ChevronRight className="size-3" />
          </Link>
        }
      />
          ...
          {d.assetExposure.map((a) => (
            <div key={a.name} className="rounded-xl border border-white/5 bg-white/[0.02] p-3">
              ...
              <div className="grid grid-cols-3 gap-2 mt-2 text-center">
                <div>
                  <div className="text-xs font-bold tabular-nums">{a.mentions}</div>
                  <div className="text-[9px] uppercase text-muted-foreground tracking-widest">
                    Mentions
                  </div>
                </div>
                <div>
                  <div className="text-xs font-bold tabular-nums" style={{ color: SEV_COLOR.High }}>
                    {a.threats}
                  </div>
                  <div className="text-[9px] uppercase text-muted-foreground tracking-widest">
                    Threats
                ...
                <div>
                  <div className="text-xs font-bold tabular-nums">{fmt(a.reach)}</div>
                  <div className="text-[9px] uppercase text-muted-foreground tracking-widest">
                    Reach
                  </div>
                </div>
...
function ScanTimeline({ d }: { d: CmdData }) {
  const iconFor: Record<string, React.ComponentType<{ className?: string }>> = {
    ...
    evidence: FileText,
    enforcement: PlayCircle,
  };
  const toneFor: Record<string, string> = {
    finding: "oklch(0.72 0.18 55)",
    evidence: "oklch(0.55 0.22 295)",
    enforcement: "oklch(0.7 0.16 155)",
  };
  return (
    <Glass className="p-5">
            ...
            {d.timeline.map((ev, i) => {
              ...
              return (
                <div key={i} className="relative flex items-start gap-3">
                  <div
                    className="absolute -left-6 top-1 size-4 rounded-full grid place-items-center"
                    style={{
        ...
        {actions.map((a) => {
          ...
          return (
            <Link
              key={a.label}
              to={a.to}
              className="rounded-xl border border-white/10 bg-white/[0.02] hover:bg-white/[0.06] transition p-3 group"
            >
              <div
                className="size-9 rounded-lg grid place-items-center"
                style={{
                  background: `color-mix(in oklab, ${a.tone} 18%, transparent)`,
                  color: a.tone,
                }}
              >
                <Icon className="size-4" />
              ...
              <div className="text-[10px] text-muted-foreground mt-0.5 group-hover:text-primary inline-flex items-center gap-1">
...
function PersonFaceProtectionRow() {
  ...
  const items: {
  ...
  }[] = [
    {
      ...
      value: s?.protectedFaces ?? 0,
      hint: "indexed references",
      color: "oklch(0.68 0.16 200)",
      to: "/face-protection",
    },
    {
    ...
    },
    {
      icon: UserX,
      label: "Impersonation",
      value: s?.impersonationAlerts7d ?? 0,
      hint: "7d threats",
      color: "oklch(0.63 0.24 25)",
      to: "/face-protection",
    },
            ...
            <div className="relative rounded-xl border border-white/10 bg-background/60 backdrop-blur-md p-4 transition hover:border-white/25">
              <div className="flex items-center gap-3">
                <div
                  ...
                  style={{ background: `${it.color.replace(")", " / 0.18)")}` }}
                >
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-2xl font-bold leading-tight" style={{ color: it.color }}>
                    {fmt(it.value)}
                  </div>
