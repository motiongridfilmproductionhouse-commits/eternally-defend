...
export function DiscoveryPanel() {
  ...
  const manualMut = useMutation({
    mutationFn: () =>
      addManualFn({ data: { subjectId: activeSubjectId!, profile_url: manualUrl.trim() } }),
    onSuccess: () => {
      setManualUrl("");
          ...
          <div className="md:col-span-1 space-y-4">
            <div className="rounded-xl border border-border p-4 space-y-2 bg-card">
              ...
              <label className="block text-xs font-semibold">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setForm({ ...form, subject_kind: e.target.value as Subject["subject_kind"] })
                  }
                  className="mt-1 w-full px-3 py-2 rounded-lg border border-border text-sm bg-card"
                >
                  {(["person", "brand", "company", "domain", "handle", "website"] as const).map(
                    (k) => (
              ...
              <label className="block text-xs font-semibold">
                ...
                <input
                  ...
                  onChange={(e) => setForm({ ...form, website_domain: e.target.value })}
              ...
              >
                {createMut.isPending || discoverMut.isPending ? (
            ...
            <div className="rounded-xl border border-border p-3 bg-card">
              ...
              {subjectsQuery.isLoading ? (
                ...
                <div className="py-6 text-center text-xs text-muted-foreground">
                  None yet — search to add your first.
                </div>
              ) : (
                <div className="space-y-1">
                  {subjects.map((s) => (
                    <div
                      key={s.id}
                      className={`flex items-center gap-2 px-2 py-1.5 rounded-md text-sm ${activeSubjectId === s.id ? "bg-accent" : "hover:bg-accent/50"}`}
                    >
                      <button
                      ...
                      >
                        ...
                        <div className="text-[10px] text-muted-foreground">
                          {s.subject_kind}
                          {s.website_domain ? ` · ${s.website_domain}` : ""}
                        </div>
                      </button>
                      <button
                        ...
                        onClick={() => {
                          if (
                            confirm(
                              `Remove subject "${s.query}"? This deletes all discovered accounts for it.`,
                            )
                          )
              ...
              <>
                <div className="flex items-center gap-2 flex-wrap">
                  <div className="text-sm font-semibold">
                    Candidates for “{activeSubject.query}”
                  </div>
                  <button
                ...
                {accountsQuery.isLoading ? (
                  ...
                  <div className="py-8 text-center text-sm text-muted-foreground">
                    No candidates yet.{" "}
                    {discoverMut.isPending ? "Discovering…" : "Try re-running discovery."}
                  </div>
                ) : (
