...
export function AutomationDrawer({
  ...
  const startMut = useMutation({
    ...
    onSuccess: (res) => {
      setJobId(res.jobId);
      toast.success(
        res.alreadyRunning ? "Attached to running automation job" : "Automation job queued",
      );
  ...
  const submitMut = useMutation({
    mutationFn: (notes: string) =>
      jobId ? mark({ data: { jobId, notes } }) : Promise.resolve(null),
    onSuccess: () => {
        ...
        <div className="p-4 space-y-5">
          {!isYouTube && (
            <div className="rounded-xl border border-amber-200 bg-amber-50 text-amber-900 p-3 text-sm flex gap-2">
              ...
              Browser automation is currently available for YouTube only. Other platforms remain
              manual.
            </div>
          )}
            ...
            <div className="space-y-3">
              <div>
                ...
                <div className="grid grid-cols-1 gap-2">
                  <label
                    className={`border rounded-lg p-3 cursor-pointer ${adapter === "youtube_copyright" ? "border-primary bg-primary/5" : "border-border"}`}
                  >
                    <input
                      type="radio"
                      name="adapter"
                      value="youtube_copyright"
                      checked={adapter === "youtube_copyright"}
                      onChange={() => setAdapter("youtube_copyright")}
                      className="mr-2"
                    />
                    ...
                    <div className="text-xs text-muted-foreground pl-6 mt-0.5">
                      Fills the official copyright removal form, uploads evidence, stops at Review.
                    </div>
                  </label>
                  <label
                    className={`border rounded-lg p-3 cursor-pointer ${adapter === "youtube_community" ? "border-primary bg-primary/5" : "border-border"}`}
                  >
                    <input
                      type="radio"
                      name="adapter"
                      value="youtube_community"
                      checked={adapter === "youtube_community"}
                      onChange={() => setAdapter("youtube_community")}
                      className="mr-2"
                    />
                    <span className="text-sm font-medium">YouTube Community Guideline Report</span>
                    <div className="text-xs text-muted-foreground pl-6 mt-0.5">
                      Reports the video via the in-player Report menu with the selected category.
                    </div>
                  </label>
                </div>
              </div>
              <div className="rounded-xl border border-blue-100 bg-blue-50/60 p-3 text-xs text-blue-900">
                Automation uses the client's stored YouTube session. No credentials are sent to the
                browser; the external worker service handles the flow and stops at the platform's
                Review screen for a human to submit.
              </div>
              <Button
          ...
          {jobId && (
            <>
              <div className="flex items-center gap-2">
                <span
                  className={`text-xs px-2 py-1 rounded-full border ${statusTone[job?.status ?? "queued"] ?? statusTone.queued}`}
                >
                  {job?.status ?? "queued"}
                </span>
                {job?.started_at && (
              ...
              {job?.error_json && (
                <div className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-900">
                  <div className="font-semibold flex items-center gap-1">
                    <AlertTriangle className="size-4" /> Automation error
                  </div>
              ...
              {reviewSummary && (
                <div className="rounded-xl border border-border p-3 space-y-2 bg-slate-50/60">
                  ...
                  {reviewSummary.client && (
                    <div className="text-sm">
                      <b>Client:</b> {reviewSummary.client}
                    </div>
                  )}
                  {reviewSummary.original && (
                    <div className="text-sm">
                      <b>Original work:</b> {reviewSummary.original}
                    </div>
                  )}
                  {reviewSummary.match && (
                    <div className="text-sm">
                      <b>Detected match:</b> {reviewSummary.match}
                    </div>
                  )}
                  ...
                  {reviewSummary.validation && (
                    <div
                      className={`text-xs rounded px-2 py-1 inline-flex items-center gap-1 ${reviewSummary.validation.ok ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}
                    >
                      {reviewSummary.validation.ok ? (
                      ...
                      {reviewSummary.validation.ok
                        ...
                        : `${reviewSummary.validation.issues?.length ?? 0} issue(s)`}
                    </div>
                  )}
                </div>
              )}
              ...
              {job?.cdp_ws_url && job.status === "review_ready" && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="block rounded-xl border border-primary/40 bg-primary/5 p-3 hover:bg-primary/10"
                >
                  ...
                  <div className="text-xs text-muted-foreground mt-1">
                    Session expires{" "}
                    {job.cdp_expires_at
                      ? new Date(job.cdp_expires_at).toLocaleTimeString()
                      : "shortly"}
                    . Click Submit inside the browser, then confirm below.
                  </div>
                </a>
              )}
                ...
                <div className="max-h-64 overflow-y-auto space-y-1">
                  ...
                  {events.map((e) => (
                    <div key={e.id} className="text-xs flex gap-2 items-baseline">
                      ...
                      {e.result && (
                        <span className={e.result === "ok" ? "text-emerald-700" : "text-red-700"}>
                          {e.result}
                        </span>
                      )}
                    </div>
              ...
              {(job?.status === "review_ready" || job?.status === "running") && (
                <div className="space-y-2">
                  <textarea
                    ...
                    rows={2}
                    className="w-full text-xs border border-border rounded-lg p-2 bg-background"
                  />
                  <div className="flex gap-2">
                    ...
                    <Button
                      size="sm"
                      onClick={() => submitMut.mutate(notes)}
