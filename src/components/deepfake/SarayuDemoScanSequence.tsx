...
export function useSarayuDemoSequence(
  key: string | null,
  enabled: boolean,
): SarayuDemoSequence {
...
function counterLabel(progress: SarayuDemoProgress): string {
  ...
  if (progress.stage === "discovery") return `${progress.queries}/39 queries · ${progress.domains} domains investigated`;
  if (progress.stage === "analysis") return `${progress.pages} pages analyzed · ${progress.faceComparisons} face comparisons`;
  if (progress.stage === "verification") return `${progress.verifiedPages} verified pages · ${progress.domains} unique domains`;
  ...
  return (
    <section className="space-y-3" data-testid="sarayu-demo-sequence">
      <div className="rounded-md border border-sky-400/30 bg-sky-500/10 px-3 py-2 text-[11px] text-sky-200">
        Demonstration scan visualization · staged presentation only; no provider calls are represented.
      </div>
      ...
      <div className="card-surface p-4" aria-live="polite">
        <div className="flex items-center justify-between gap-3">
          <div>
            <div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Current stage</div>
            <div className="mt-1 text-sm font-semibold">{progress.stage === "complete" ? "Evidence Classification" : STAGES[currentIndex]?.title}</div>
            <div className="mt-0.5 text-xs text-muted-foreground">{progress.stage === "complete" ? "Investigation complete" : STAGES[currentIndex]?.copy}</div>
          </div>
          <div className="flex size-14 items-center justify-center rounded-full border-4 border-sky-400/20 text-sm font-semibold text-sky-300" style={{ borderTopColor: "rgb(56 189 248 / 0.9)" }}>
            {Math.round(progress.progress * 100)}%
          </div>
        </div>
        <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">
          {STAGES.map((stage, index) => (
            <div key={stage.id} className={`rounded border px-2 py-2 text-[11px] ${index < currentIndex || progress.stage === "complete" ? "border-emerald-400/30 bg-emerald-500/10 text-emerald-300" : index === currentIndex ? "border-sky-400/40 bg-sky-500/10 text-sky-200" : "border-border/60 text-muted-foreground"}`}>
              {index < currentIndex || progress.stage === "complete" ? "Complete · " : index === currentIndex ? "Active · " : "Queued · "}{stage.title}
            </div>
          ))}
        </div>
        <div className="mt-3 text-xs text-muted-foreground">{counterLabel(progress)}</div>
        {sequence.active ? (
          <Button type="button" variant="outline" size="sm" className="mt-4" onClick={sequence.skip} data-testid="sarayu-demo-skip">
            <SkipForward className="mr-1.5 size-3.5" /> Skip animation
          </Button>
        ) : sequence.complete ? (
          <Button type="button" variant="ghost" size="sm" className="mt-4" onClick={sequence.replay} data-testid="sarayu-demo-replay">
