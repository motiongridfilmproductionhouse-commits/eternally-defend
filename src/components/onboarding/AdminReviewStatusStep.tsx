...
export function AdminReviewStatusStep({
  ...
  const signedDoc =
    authBundle?.documents?.find((d: any) => d.kind === "signed" && d.version === auth.version) ||
      ...
      <CardContent className="space-y-6">
          ...
          <div className="flex flex-col items-center justify-center py-6 space-y-4 bg-blue-500/10 border border-blue-500/20 rounded-xl">
            ...
            <div className="text-center">
              ...
              <div className="text-sm text-blue-200 mt-1 max-w-sm">
                Your submission is awaiting review. We will update this page when a decision or
                additional request is available.
              </div>
            </div>
          </div>
        )}
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-orange-200 mt-1">
                  Our team has requested more information or corrections before your authorization
                  can be approved.
                </div>
                {latestReview?.notes && (
                ...
                <p className="text-xs text-orange-300 mt-3">
                  After making the requested changes, you must regenerate the draft and sign it
                  again.
                </p>
              </div>
            </div>
        ...
        {(isRejected || isSuspended || isRevoked) && (
          <div className="flex flex-col py-6 px-4 space-y-4 bg-red-500/10 border border-red-500/20 rounded-xl">
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="text-lg font-semibold text-red-400 capitalize">
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-4 space-y-3">
          ...
          <div className="flex justify-between items-center text-sm">
            ...
            <span className={kycStatus === "APPROVED" ? "text-emerald-400" : "text-white/50"}>
              {kycStatus}
            </span>
          </div>
          <div className="flex justify-between items-center text-sm">
            <span className="text-white/70">Face Verification</span>
            <span className={faceStatus === "FACE_VERIFIED" ? "text-emerald-400" : "text-white/50"}>
              {faceStatus}
            </span>
          </div>
          <div className="flex justify-between items-center text-sm">
            <span className="text-white/70">Asset Verification</span>
            <span className={assetStatus === "VERIFIED" ? "text-emerald-400" : "text-white/50"}>
              {assetStatus}
            </span>
          </div>
          <div className="flex justify-between items-center text-sm">
