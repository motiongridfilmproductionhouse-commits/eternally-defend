  ...
  return (
    <div className="space-y-4 rounded-xl border border-border p-4 bg-accent/20">
      ...
      <div className="text-xs text-muted-foreground">
        Paste a channel URL, @handle, channel ID (UC…), or channel name. We'll resolve it through
        the YouTube Data API before saving.
      </div>
      ...
      {results && results.length > 0 && (
        <div className="space-y-2">
          <div className="flex items-center justify-between text-[11px] uppercase tracking-wider text-muted-foreground">
            <span>
              {results.length} candidate{results.length === 1 ? "" : "s"} · matched by {strategy}
            </span>
            {strategy === "search" && results.length < 10 && (
              <button className="underline" onClick={() => runSearch(10)}>
                Show more matches
              </button>
            )}
          </div>
...
function ChannelCard({
  ...
  channel: YTChannel;
  onConfirm: (c: YTChannel) => Promise<void> | void;
}) {
  ...
  const subs = channel.hidden_subscriber_count
    ...
    <div className="border border-border rounded-lg p-3 bg-background flex gap-3">
      {channel.profile_image_url ? (
        <img
          ...
          alt=""
          className="size-14 rounded-full object-cover shrink-0 border border-border"
        />
      ) : (
        <div className="size-14 rounded-full bg-muted grid place-items-center shrink-0">
      ...
      <div className="flex-1 min-w-0 space-y-1.5">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
          <span className="flex items-center gap-1">
            <Users className="size-3" /> {subs}
          </span>
          <span className="flex items-center gap-1">
            <PlaySquare className="size-3" /> {compact(channel.video_count)} videos
          </span>
          <span className="flex items-center gap-1">
            <Eye className="size-3" /> {compact(channel.total_view_count)} views
          </span>
        </div>
        {channel.description && (
...
export function YouTubeAssetCard({
  asset,
  onRefresh,
  onRemove,
}: {
  ...
  const m = (asset.metadata ?? {}) as Partial<YTChannel> & {
    confirmation_status?: string;
    verification_status?: string;
    last_synced_at?: string;
  };
  const subs = m.hidden_subscriber_count
    ? "Subscriber count hidden"
    : `${compact(m.subscriber_count ?? null)} subscribers`;
    ...
    <div className="border border-border rounded-xl p-4 bg-background">
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-muted-foreground truncate">
            {m.channel_handle ?? asset.value ?? m.channel_id}
          </div>
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground mt-1">
            ...
            <span className="flex items-center gap-1">
              <PlaySquare className="size-3" /> {compact(m.video_count ?? null)} videos
            </span>
            <span className="flex items-center gap-1">
              <Eye className="size-3" /> {compact(m.total_view_count ?? null)} views
            </span>
          </div>
