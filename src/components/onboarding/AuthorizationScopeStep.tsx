...
const SCOPES = [
  ...
  {
    key: "submit_final_after_approval",
    label: "Final complaint submission only after separate approval",
  },
] as const;
    ...
    <Card className="bg-[#0A1128] border-white/10 text-white shadow-2xl shadow-black/50">
      <CardHeader>
        ...
        <CardDescription className="text-white/60">
          Select the permissions you wish to grant Eterna AI to act on your behalf. We will only act
          within these specific permissions.
        </CardDescription>
      </CardHeader>
          ...
          <>
            <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 flex gap-3 text-sm text-blue-200">
              ...
              <div>
                ...
                Eterna AI requires at least one authorization scope to function. Note that enabling
                complaint preparation does not mean automatic submission; final platform submission
                requires your separate manual approval for each case.
              </div>
            </div>
                  ...
                  <div className="text-sm text-white/90 leading-relaxed">
                    ...
                    {s.key === "submit_final_after_approval" && (
                      <span className="block text-xs text-orange-400 mt-1 flex items-center gap-1">
                        <AlertTriangle className="size-3" /> Eterna will never submit without your
                        review.
                      </span>
                    )}
                  </div>
            ...
            <div className="flex items-center justify-between bg-black/20 rounded-lg p-3 border border-white/5 text-sm">
              <div className="text-white/60">
                Selected: <strong className="text-white">{selectedCount}</strong> / {SCOPES.length}{" "}
                permissions
              </div>
              <div>
                {saveStatus === "SAVING" && (
