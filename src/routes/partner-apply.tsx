...
function PartnerApplyPage() {
  ...
  const canSubmit =
    ...
    decl.authority &&
    decl.accurate &&
    decl.commercial_terms &&
    decl.no_incentives &&
    decl.data_protection;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: currentAuth, error: sessionError } = await supabase.auth.getSession();
      ...
      if (!currentAuth.session) {
        ...
        if (anonymousError) {
          throw new Error(`Unable to start application session: ${anonymousError.message}`);
        }
      }
      ...
      await submit({
        data: {
          ...
          expected_monthly_clients: form.expected_monthly_clients
            ? Number(form.expected_monthly_clients)
            : null,
    ...
    <div className="min-h-screen bg-white text-slate-900">
      <header className="border-b border-slate-200">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <Link to="/auth" className="text-sm text-slate-500 hover:text-slate-800">
            Back to sign in
          </Link>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-6 py-10 space-y-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <Section icon={Building2} title="People & Contact">
            <Grid>
              <Field label="Founder's full name *">
                <Input
              ...
              <Field label="Representative job title">
                <Input value={form.rep_title} onChange={(e) => set("rep_title", e.target.value)} />
              </Field>
            ...
            <Grid>
              ...
              <Field label="Expected clients / month">
                <Input
                  type="number"
                  min={0}
                  value={form.expected_monthly_clients}
                  onChange={(e) => set("expected_monthly_clients", e.target.value)}
                />
              </Field>
              <Field label="Partnership type *">
                <select
                  ...
                  onChange={(e) =>
                    set("partnership_type", e.target.value as Form["partnership_type"])
                  }
                >
          ...
          <Section icon={FileSignature} title="Declarations & Electronic Signature">
            <div className="space-y-3 text-sm">
              <Decl
                label="I have the authority to sign on behalf of the company named above."
                checked={decl.authority}
                onChange={(v) => setDecl((d) => ({ ...d, authority: v }))}
              />
              <Decl
                label="All information and documents I have provided are true and accurate."
                checked={decl.accurate}
              ...
              />
              <Decl
                label="I will not offer cashbacks, kickbacks or incentives to prospects to induce sign-ups."
                checked={decl.no_incentives}
                onChange={(v) => setDecl((d) => ({ ...d, no_incentives: v }))}
              />
              <Decl
                label="I will process personal data in line with applicable data-protection law and keep Eterna and client information confidential."
                checked={decl.data_protection}
                onChange={(v) => setDecl((d) => ({ ...d, data_protection: v }))}
              />
            </div>
            <div className="mt-6 max-w-md">
              ...
              <p className="mt-2 text-xs text-slate-500">
                By submitting, you sign the Eterna Partner Application electronically. A draft
                Partner Agreement / MOU will be generated automatically and marked{" "}
                <strong>Draft — Awaiting Eterna Approval</strong>.
              </p>
            </div>
          </Section>
          ...
          <div className="flex items-center justify-between border-t border-slate-200 pt-6">
            <div className="text-xs text-slate-500">
              Your application will be marked <strong>Pending Review</strong>.
            </div>
            <Button
              type="submit"
    ...
    <label className="block cursor-pointer border border-dashed border-slate-300 rounded-lg p-4 hover:border-blue-500 transition">
      ...
      <div className="mt-2 text-sm">
        {busy ? (
          ...
          <span className="text-green-600">✓ {uploadedName}</span>
        ) : (
          <span className="text-slate-500">Click to select file (PDF / JPG / PNG)</span>
        )}
      </div>
      <input
        type="file"
