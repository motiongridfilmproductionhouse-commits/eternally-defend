...
export const Route = createFileRoute("/_app/scan")({
  ...
  ): { assetId?: string; query?: string; auto?: string } => ({
      ...
      try {
        const mapped = report.hits.map((h) => ({
          ...
          sourceMetadata: {
            ...
            // Which discovery provider(s) actually found this hit — see
            // RawHit.provider / ScanHit.discoveredByProviders in
            // src/routes/api/scan.ts. [] means the pipeline ran but no
            // provider tag was captured for this hit, never "unknown".
    ...
    try {
      const result = await generateReportPdf({
        data: {
          ...
          exportType: "PRELIMINARY_INTELLIGENCE",
          hits: report.hits.map((h) => ({
            ...
            category: h.category,
            contentLabel: h.contentLabel,
            severity: h.severity,
            sentiment: h.sentiment,
            threatScore: h.threatScore,
            credibilityScore: h.credibilityScore,
            reachEstimate: h.reachEstimate,
            engagement: h.engagement,
            detectionReason: h.detectionReason,
            recommendedAction: h.recommendedAction,
            discoveredAt: h.discoveredAt,
            thumbnailUrl: h.media?.thumbnailHi ?? h.media?.thumbnail ?? null,
            videoId: h.media?.videoId ?? null,
            channelId: h.media?.channelId ?? null,
            channelUrl: h.media?.channelUrl ?? null,
            views: h.media?.viewsAvailable ? (h.media.views ?? 0) : null,
            likes: h.media?.likesAvailable ? (h.media.likes ?? 0) : null,
            comments: h.media?.commentsAvailable ? (h.media.comments ?? 0) : null,
            viewsAvailable: h.media?.viewsAvailable === true,
            likesAvailable: h.media?.likesAvailable === true,
            commentsAvailable: h.media?.commentsAvailable === true,
            statisticsCapturedAt: h.discoveredAt,
            statisticsSource: h.platform === "YouTube" ? "YouTube Data API" : null,
            transcript: null,
            transcriptSource: null,
            transcriptConfidence: null,
            review: {
              reviewStatus: "AUTOMATED_LEAD" as const,
              reviewerName: null,
              reviewerRole: null,
              reviewedAt: null,
              targetPerson: q.trim() || report.query,
              exactOriginalStatement: null,
              statementLanguage: null,
              verifiedEnglishTranslation: null,
              videoStartTimestamp: null,
              videoEndTimestamp: null,
              speakerIdentity: h.author ?? null,
              contentContext: null,
              contentPosition: h.contentPosition ?? "UNKNOWN",
              statementType: "UNKNOWN" as const,
              allegedViolationTypes: [],
              ...
              recommendedAction: "Preserve and conduct human review",
              reviewerNotes: null,
              reviewerDeclarationSigned: false,
            },
            scoring: h.scoring ?? null,
            evidence: {
              fullPageScreenshotPreserved: false,
              originalMediaPreserved: false,
              hashesGenerated: false,
              chainOfCustodyCreated: false,
              translationRequired: false,
              translationVerified: false,
              channelIdentifiersPreserved: Boolean(h.media?.channelId),
              ownershipProofPreserved: false,
            },
          })),
        },
      });
      ...
      toast.success(
        "Preliminary evidence report opened — human verification is required before legal submission",
      );
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-border bg-card">
        <div
          ...
          style={{
            background:
              "radial-gradient(600px 200px at 10% 0%, oklch(0.85 0.18 295 / 0.35), transparent 60%), radial-gradient(500px 220px at 90% 100%, oklch(0.85 0.18 320 / 0.35), transparent 60%)",
          }}
        />
        <div className="relative p-6 md:p-8">
          ...
          <h1 className="mt-2 text-2xl md:text-3xl font-display font-bold leading-tight">
            Evidence-based scan across{" "}
            <span className="text-gradient-brand">news, video, social & forums</span>
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            Enter a person, brand, or organization and generate a full, source-linked reputation
            report.
          </p>
          ...
          <form onSubmit={submit} className="mt-5 grid grid-cols-1 md:grid-cols-2 gap-3">
            <Field label="Name / Brand / Organization *">
              <div className="relative">
                ...
                <input
                  ...
                  required
                  placeholder='e.g. "Eterna AI"'
                  className="w-full pl-10 pr-3 py-2.5 rounded-xl border border-border bg-card text-sm focus:outline-none focus:ring-2 focus:ring-primary/25"
                />
              </div>
            </Field>
            ...
            <Field label="Hashtags to track">
              <input
                ...
                placeholder="#brandname, #topic"
                className="w-full px-3 py-2.5 rounded-xl border border-border bg-card text-sm"
              />
            </Field>
            <div className="md:col-span-2">
              <div className="text-[10px] tracking-[0.16em] font-semibold text-muted-foreground mb-2">
                Scan period
              </div>
            ...
            <div className="flex items-end">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-white text-sm font-semibold disabled:opacity-60 shadow-lg"
        ...
        <>
            ...
            <div className="lg:col-span-2 rounded-2xl border border-border bg-card p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-lg font-display font-bold mt-1">
                ...
                <button
                  onClick={exportPdf}
                  disabled={pdfPending}
                  className="text-xs px-3 py-1.5 rounded-lg border border-border hover:bg-accent inline-flex items-center gap-1.5 disabled:opacity-60"
                  title="Downloads a preliminary intelligence report. Human review is required before legal or police submission."
                >
                  {pdfPending ? (
                  ...
                  {pdfPending ? "Building PDF..." : "Open / Download Preliminary Evidence PDF"}
                </button>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4 text-xs">
                <Fact
                  label="Most damaging topic"
                  value={report.executiveSummary.mostDamagingTopic}
                />
                <Fact
                  label="Most influential source"
                  value={report.executiveSummary.mostInfluentialSource}
                />
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                <ActionList
                  ...
                  items={report.executiveSummary.immediateActions}
                />
                <ActionList
                  title="Long-term recommendations"
                  items={report.executiveSummary.longTerm}
                />
              </div>
              <div className="mt-3 text-[11px] text-muted-foreground italic">
                This score is an analytical estimate based on public content and is not a legal
                finding.
              </div>
            </div>
            <div
              ...
              style={{
                background: `linear-gradient(135deg, color-mix(in oklab, ${scoreColor(report.reputationScore)} 18%, white), white)`,
              }}
            >
              <div className="text-[10px] tracking-[0.18em] font-semibold text-muted-foreground">
                REPUTATION SCORE
          ...
          <PageCard title="METHODOLOGY & LIMITATIONS" sub="How Eterna AI produced this report">
            <ul className="text-xs text-muted-foreground list-disc pl-5 space-y-1">
              ...
              <li>
                Unique results: {report.totals.unique} · duplicates removed:{" "}
                {report.totals.duplicatesRemoved}
              </li>
              <li>
                Only public web content is collected via Firecrawl; no private accounts, paywalls,
                or authenticated pages are accessed.
              </li>
              <li>
                Threat and reputation scores are analytical estimates, not legal findings.
                Allegations are labeled as such and are not treated as verified facts.
              </li>
      ...
      {!report && !m.isPending && (
        <PageCard title="HOW IT WORKS" sub="Powered by Firecrawl + Eterna AI risk model">
          <ol className="text-sm text-muted-foreground space-y-2 list-decimal pl-5">
            ...
            <li>
              Eterna AI runs parallel searches, classifies each hit for category, sentiment,
              credibility, reach, and threat, then computes an overall Reputation Score.
            </li>
            <li>
              Review findings by section, export as CSV, or promote high-risk items to the Threat
...
function EternaStatusCard({ report }: { report: ReportWithDiagnostics }) {
  ...
  const socialFindings = hits.filter((h) =>
    ["Reddit", "Instagram", "X", "TikTok", "Facebook"].includes(h.source),
  ).length;
  const webFindings = hits.filter((h) => h.source === "Web").length;
  const otherFindings = Math.max(
    0,
    hits.length - videoFindings - newsFindings - socialFindings - webFindings,
  );
  ...
  const hasPartialCoverage =
    !DEMO_MODE && (diag?.youtube?.quotaExhausted || diag?.youtube?.status === "quota_exhausted");

  const scanTime = scannedAt
    ? new Date(scannedAt).toLocaleTimeString(undefined, {
        hour: "2-digit",
        minute: "2-digit",
        second: "2-digit",
      })
    ...
    <div className="rounded-2xl border border-border bg-card px-6 py-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        {(
          ...
          ] as { label: string; val: string; accent: boolean }[]
          ...
          >
            <div
              className={`text-xl font-bold font-display ${accent ? "text-red-500" : "text-foreground"}`}
            >
              {val}
            </div>
      ...
      {hasPartialCoverage ? (
        <p className="text-[11px] text-muted-foreground border-t border-border pt-3 mt-1">
          ⓘ Some sources are temporarily limited. Eterna continued scanning using alternate
          discovery sources.
        </p>
      ) : (
  ...
  return (
    <details className="rounded-2xl border border-dashed border-amber-300 dark:border-amber-700/50 bg-amber-50/30 dark:bg-amber-950/10 px-5 py-3">
      ...
      <div className="mt-4">
        ...
        {(
          diag as {
            monthWindow?: { filter: string; label: string; startIso: string; endIso: string };
          }
        ).monthWindow && (
          <div className="mt-3 text-[11px] text-muted-foreground">
            Scan range:{" "}
            {
              (diag as { monthWindow?: { label: string; startIso: string; endIso: string } })
                .monthWindow?.label
            }{" "}
            ·{" "}
            {(diag as { monthWindow?: { startIso: string; endIso: string } }).monthWindow?.startIso}{" "}
            → {(diag as { monthWindow?: { endIso: string } }).monthWindow?.endIso}
          </div>
        )}
      </div>
    </details>
...
// ---------------------------------------------------------------------------
function getClientMonthLabel(filter: "this" | "previous" | "twoAgo"): {
  label: string;
  sub: string;
} {
  const now = new Date();
        ...
        return (
          ...
          >
            <span
              ...
              style={active ? { opacity: 0.85 } : { color: "var(--color-muted-foreground)" }}
            >
              {range.title}
            </span>
    ...
    <div className="space-y-3">
      ...
      {ytDiag && (
        ...
        >
            ...
            <div className="flex items-center gap-2">
              {ytQuotaExhausted ? (
                <>
                  ...
                  {ytDiag.quotaReason && (
                    <span className="text-[10px] text-red-400 ml-1">({ytDiag.quotaReason})</span>
                  )}
                </>
              ) : (
                <>
            ...
            <div className="text-[11px] text-muted-foreground">
              {ytQuotaExhausted ? (
                <span className="text-red-500 font-semibold">
                  Firecrawl Discovery Mode activated automatically
                </span>
              ) : (
                <span>
                  <strong className="text-foreground">{ytDiag.videosFound}</strong> videos found
                </span>
              )}
            </div>
          </div>
          <div className="flex flex-wrap gap-x-5 gap-y-1 text-[11px] text-muted-foreground">
            <span>
              Queries run: <strong className="text-foreground">{ytDiag.queriesRun}</strong>
            </span>
            <span>
              Pages scanned: <strong className="text-foreground">{ytDiag.pagesScanned}</strong>
            </span>
            <span>
              Videos found:{" "}
              <strong className={ytQuotaExhausted ? "text-red-500" : "text-foreground"}>
                {ytDiag.videosFound}
              </strong>
            </span>
            {ytDiag.apiErrors > 0 && (
              <span className="text-amber-500 font-semibold">API errors: {ytDiag.apiErrors}</span>
            )}
          </div>
          {ytQuotaExhausted && (
            <div className="mt-3 text-[11px] text-red-600 dark:text-red-400 bg-red-100/60 dark:bg-red-900/20 rounded-lg px-3 py-2">
              ⚡ YouTube Data API daily quota exhausted. All YouTube content is now discovered
              through Firecrawl's public web index (site:youtube.com searches). Results are
              normalized and displayed identically.
            </div>
          )}
        </div>
      )}
      ...
      {fcActive && fcDiscovery && (
        <div className="rounded-2xl border border-blue-200 dark:border-blue-800/40 bg-blue-50/30 dark:bg-blue-950/10 px-5 py-4">
          <div className="flex items-center justify-between mb-3 gap-4 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-bold tracking-wider text-blue-600 dark:text-blue-400 uppercase">
                Firecrawl Discovery Mode · Active
              </span>
            </div>
            <div className="text-[11px] text-muted-foreground">
              <strong className="text-foreground">{fcDiscovery.uniqueUrls ?? 0}</strong> unique URLs
              discovered ·{" "}
              <strong className="text-foreground">{fcDiscovery.queriesExecuted ?? 0}</strong>{" "}
              queries executed
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3">
            {(
              [
                {
                  label: "YouTube URLs",
                  val: fcDiscovery.youtubeUrlsDiscovered ?? 0,
                  color: "text-red-500",
                },
                { label: "News", val: fcDiscovery.newsDiscovered ?? 0, color: "text-blue-600" },
                {
                  label: "Social/Web",
                  val: fcDiscovery.socialDiscovered ?? 0,
                  color: "text-purple-600",
                },
                {
                  label: "Other Web",
                  val: fcDiscovery.otherWebDiscovered ?? 0,
              ...
              <div
                key={label}
                className="rounded-lg bg-white/60 dark:bg-white/5 border border-blue-100 dark:border-blue-800/30 p-2 text-center"
              >
          ...
          <div className="flex flex-wrap gap-x-5 gap-y-1 text-[11px] text-muted-foreground">
            <span>
              Tier-1 queries:{" "}
              <strong className="text-foreground">{fcDiscovery.tier1Queries ?? 0}</strong>
            </span>
            <span>
              Tier-2 (expanded):{" "}
              <strong className="text-foreground">{fcDiscovery.tier2Queries ?? 0}</strong>
            </span>
            <span>
              YouTube web queries:{" "}
              <strong className="text-foreground">{fcDiscovery.ytWebQueries ?? 0}</strong>
            </span>
            <span>
              Raw fetched: <strong className="text-foreground">{fcDiscovery.rawUrls ?? 0}</strong>
            </span>
          </div>
      ...
      <div className="rounded-2xl border border-border bg-card px-5 py-4">
        <div className="flex items-center justify-between mb-3 gap-4 flex-wrap">
          ...
          <div className="text-[11px] text-muted-foreground">
            <span className="font-semibold text-foreground">{totalRaw.toLocaleString()}</span> raw
            results across{" "}
            <span className="font-semibold text-foreground">{sourcesWithResults.length}</span>{" "}
            source{sourcesWithResults.length !== 1 ? "s" : ""}
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
          {entries.map(([src, count]) => (
            <div
              key={src}
              className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-border bg-background text-[11px] font-semibold"
            >
              <span className="text-muted-foreground">
                {SOURCE_ICONS[src] ?? <Globe className="size-3.5" />}
              </span>
              <span>{src}</span>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <div className="text-[10px] tracking-[0.16em] font-semibold text-muted-foreground mb-1.5">
        {label}
      </div>
      {children}
    </label>
...
function KPI({
  ...
  tone: "brand" | "danger" | "warn" | "viral";
}) {
  const bg =
    tone === "danger"
      ? "linear-gradient(135deg, oklch(0.96 0.06 25), oklch(0.94 0.1 25 / 0.6))"
      : tone === "warn"
        ? "linear-gradient(135deg, oklch(0.96 0.06 70), oklch(0.94 0.1 70 / 0.6))"
        : tone === "viral"
          ? "linear-gradient(135deg, oklch(0.94 0.08 35), oklch(0.94 0.1 55 / 0.6))"
  ...
  return (
    <div className="rounded-2xl p-4 border border-border" style={{ background: bg }}>
      <div className="flex items-center justify-between">
        ...
        <span className="size-7 grid place-items-center rounded-full bg-white/70" style={{ color }}>
          {icon}
        </span>
      </div>
      <div className="mt-2 text-2xl font-display font-bold" style={{ color }}>
        {value}
      </div>
    </div>
  );
...
type SortKey =
  "newest" | "critical" | "viral" | "growth" | "reach" | "discussed" | "shared" | "threat";
...
function Bucket({
  ...
  const filtered = useMemo(() => {
    let list =
      sentimentFilter === "All" ? hits : hits.filter((h) => h.sentiment === sentimentFilter);
    const by =
      <T,>(fn: (h: ScanHit) => T) =>
      (a: ScanHit, b: ScanHit) =>
    ...
    switch (sort) {
      ...
      case "newest":
        ...
        break;
      case "reach":
        list = [...list].sort(by((h) => h.reachEstimate));
        break;
      case "growth":
        list = [...list].sort(by((h) => h.media?.growthPerDay ?? 0));
        break;
      case "viral":
        list = [...list].sort(by((h) => h.viralityScore + (h.viral ? 20 : 0)));
        break;
      case "critical":
        list = [...list].sort(by((h) => (SEV_RANK[h.severity] ?? 0) * 100 + h.threatScore));
        break;
      case "discussed":
        list = [...list].sort(by((h) => h.media?.comments ?? h.engagement));
        break;
      case "shared":
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {shown.map((h) => (
          <ResultCard
            key={h.id + h.url}
            h={h}
            added={added.has(h.url)}
            onPromote={() => onPromote(h)}
            entityTerms={entityTerms}
            scanId={scanId}
            analysisPending={!!(h.media?.videoId && analyzingVideos.has(h.media.videoId))}
          />
        ))}
      </div>
...
function ResultCard({
  ...
  const rawThumb =
    h.media?.thumbnailHi ||
    h.media?.thumbnail ||
    (isYouTube ? youtubeThumbFromUrl(h.url, "maxres") : null);
  ...
  const publishedLabel = publishedTs
    ? new Date(publishedTs).toLocaleDateString(undefined, {
        year: "numeric",
        month: "short",
        day: "numeric",
      })
  ...
  const ageLabel = !publishedTs
    ...
    : ageDays < 1
      ...
      : ageDays < 30
        ...
        : ageDays < 365
          ? `${Math.round(ageDays / 30)}mo ago`
          : `${Math.round(ageDays / 365)}y ago`;
  const m = h.media;
  const growth = m?.growthPerDay ?? 0;
  const trend: { label: string; color: string } | null =
    growth >= 20000
      ? { label: "Exploding", color: "oklch(0.63 0.24 25)" }
      : growth >= 5000
        ? { label: "Rising", color: "oklch(0.7 0.2 35)" }
        : growth >= 500
          ? { label: "Growing", color: "oklch(0.75 0.16 70)" }
          : publishedTs
            ? { label: "Steady", color: "oklch(0.68 0.16 155)" }
            : null;
    ...
    <div className="group relative rounded-2xl border border-border bg-card overflow-hidden hover:shadow-lg transition-shadow flex flex-col">
      ...
      {thumb && imgOk ? (
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="relative block aspect-video bg-gradient-to-br from-muted/60 to-secondary/60 overflow-hidden"
        >
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 px-4 text-center">
            {favicon ? (
              <img
                src={favicon}
                alt=""
                className="size-10 rounded-md bg-white/80 p-1.5 shadow-sm"
              />
            ) : (
            ...
            <div className="text-[11px] font-semibold text-foreground/80 truncate max-w-full">
              {host ?? h.platform}
            </div>
      ...
      <div className="p-5 flex-1 flex flex-col gap-3">
        <div>
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="block text-base font-semibold leading-snug line-clamp-3 hover:underline"
          >
            {displayTitle}
          </a>
          <div className="mt-1.5 flex items-center gap-2 text-[11px] text-muted-foreground flex-wrap">
            {m?.channelUrl ? (
              ...
              >
                {m.channelTitle ?? h.author ?? h.platform}
              </a>
            ) : (
            ...
            {trend && (
              <span
                ...
                style={{
                  background: `color-mix(in oklab, ${trend.color} 14%, white)`,
                  color: trend.color,
                }}
              >
        ...
        {m && (m.views || m.likes || m.comments) ? (
          <div className="grid grid-cols-4 gap-1.5 pt-1">
            ...
            <Stat
              ...
              label="Comments"
              value={fmt(m.comments ?? 0)}
            />
            <Stat
              icon={<Flame className="size-3" />}
              label="Growth/day"
              value={fmt(m.growthPerDay ?? 0)}
            />
          </div>
        ) : (
          <div className="grid grid-cols-3 gap-1.5 pt-1">
            <Stat
              ...
              label="Reach"
              value={`~${fmt(h.reachEstimate)}`}
            />
            <Stat
              icon={<TrendingUp className="size-3" />}
              label="Engagement"
              value={fmt(h.engagement)}
            />
            <Stat
              icon={<Flame className="size-3" />}
              label="Virality"
              value={`${h.viralityScore}`}
            />
          </div>
        )}
        ...
        */}
        ...
        <div className="flex items-center gap-2 mt-auto flex-wrap">
          <button
            onClick={() => setOpen((v) => !v)}
            className="flex-1 text-xs px-3 py-2 rounded-lg border border-border hover:bg-accent inline-flex items-center justify-center gap-1"
          >
            <ExternalLink className="size-3.5" /> {open ? "Hide" : "View"} evidence
          </button>
          {isYouTube && h.media?.videoId && (
            <button
              onClick={() => setMoments((v) => !v)}
              className="flex-1 text-xs px-3 py-2 rounded-lg border border-border hover:bg-accent inline-flex items-center justify-center gap-1"
            >
          ...
          <div className="mt-2 rounded-xl border border-border bg-muted/30 p-4 space-y-3">
            ...
            {m && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                ...
                {m.engagementRate != null && (
                  <EvidenceRow label="Eng. rate" value={`${m.engagementRate}%`} />
                )}
                {m.growthPerDay != null && (
                  <EvidenceRow label="Views/day" value={fmt(m.growthPerDay)} />
                )}
              </div>
            )}
            {h.whyItMatters && (
            ...
            <div>
              ...
              <div className="text-xs">
                Reputation risk {h.reputationRisk}/100 · Threat {h.threatScore}/100 · Credibility{" "}
                {h.credibilityScore}/100 · {h.sentiment} sentiment · Category: {h.category}.
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {h.legalTakedownPotential != null && (
                  <div className="rounded-lg border border-dashed border-border p-2.5 bg-muted/30">
                    <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">
                      Legal Takedown Potential
                    </div>
                    <div
                      className="text-sm font-bold mt-0.5"
                      style={{
                        color:
                          h.legalTakedownPotential >= 70
                            ? "oklch(0.63 0.24 25)"
                            : h.legalTakedownPotential >= 40
                              ? "oklch(0.7 0.18 55)"
                              : "oklch(0.55 0.03 275)",
                      }}
                    >
                      {h.legalTakedownPotential}/100
                    </div>
                  </div>
                )}
                {h.copyrightEnforcementPotential != null && (
                  <div className="rounded-lg border border-dashed border-border p-2.5 bg-muted/30">
                    ...
                    <div
                      ...
                      style={{
                        color:
                          h.copyrightEnforcementPotential >= 70
                            ? "oklch(0.63 0.24 25)"
                            : h.copyrightEnforcementPotential >= 40
                              ? "oklch(0.7 0.18 55)"
                              : "oklch(0.55 0.03 275)",
                      }}
                    >
                      {h.copyrightEnforcementPotential}/100
                    </div>
                  </div>
                )}
              </div>
...
function ScoreCard({ label, value, accent }: { label: string; value: number; accent: string }) {
  return (
    <div
      ...
      style={{ background: `color-mix(in oklab, ${accent} 6%, white)` }}
    >
...
function PersistedResults({
  ...
  const load = async (nextCursor: typeof cursor) => {
    ...
    try {
      ...
      setItems((prev) =>
        nextCursor ? [...prev, ...(res.items as PersistedHit[])] : (res.items as PersistedHit[]),
      );
  ...
  const displayItems = useMemo(() => {
    ...
    const matchesQuick = (h: PersistedHit) => {
      ...
      const hay =
        `${h.risk_type ?? ""} ${h.narrative_claim ?? ""} ${(h.tags ?? []).join(" ")} ${h.title ?? ""}`.toLowerCase();
      ...
      if (quickFilter === "deepfake")
        return /deepfake|face[- ]?swap|synthetic|ai[- ]?generated/.test(hay);
  ...
  const bulkHide = async () => {
    ...
    try {
      await Promise.all(
        ids.map((id) => hideFn({ data: { scanHitId: id, reason: "bulk_hidden" } })),
      );
    ...
    <>
      ...
      >
          ...
          {SOURCE_PRIORITY.map((s) => {
            ...
            return (
              ...
              >
                ...
                {count > 0 && (
                  <span
                    className={`text-[10px] font-bold ${active ? "text-white/90" : "text-muted-foreground"}`}
                  >
                    {count}
                  </span>
                )}
        ...
        <div className="mt-2 flex items-center justify-center text-xs text-muted-foreground">
          {loading ? (
            ...
            <button
              onClick={() => load(cursor)}
              className="px-4 py-2 rounded-full border border-border hover:bg-accent font-semibold"
            >
              Load more
            </button>
          ) : items.length > 0 ? (
      ...
      {selected.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 rounded-2xl border border-border bg-card shadow-2xl px-4 py-3 flex items-center gap-3">
          ...
          <button
            onClick={bulkHide}
            className="text-xs px-3 py-1.5 rounded-lg border border-border hover:bg-accent inline-flex items-center gap-1 text-destructive"
          >
            <EyeOff className="size-3.5" /> Hide
          </button>
          <button
            onClick={clearSelection}
            className="text-xs px-2 py-1.5 rounded-lg hover:bg-accent inline-flex items-center gap-1"
