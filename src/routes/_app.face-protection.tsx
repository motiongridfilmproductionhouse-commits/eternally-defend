...
function FaceProtection() {
  ...
  const reviewMut = useMutation({
    mutationFn: (v: {
      id: string;
      decision: "authorized" | "harmless" | "threat_created" | "dismissed";
      ...
      <header className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            AWS Rekognition-backed facial protection & review workflow
          </p>
        </div>
      </header>
      ...
      <section className="card-surface p-5">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold flex items-center gap-2">
            <ShieldCheck className="size-4" /> Face Match Review Queue
          </h2>
          <div className="flex gap-1">
            {(["pending", "authorized", "harmless", "threat_created", "dismissed"] as const).map(
              (s) => (
                <button
                  key={s}
                  onClick={() => setStatus(s)}
          ...
          <div className="space-y-3">
            {(matchesQuery.data ?? []).map((m) => (
              <MatchRow
                key={m.id}
                m={m}
                onDecide={(dec, cat, notes) =>
                  reviewMut.mutate({ id: m.id, decision: dec, category: cat, notes })
                }
              />
            ))}
        ...
        <p className="text-[11px] text-muted-foreground mt-4">
          A face match alone never creates a threat. To create a threat you must set a category and
          provide review notes (similarity ≥ 80 required).
        </p>
      </section>
    </div>
  );
...
function MatchRow({
  ...
  m: MatchRow;
  onDecide: (
    d: "authorized" | "harmless" | "threat_created" | "dismissed",
    cat: (typeof CATEGORIES)[number]["id"] | undefined,
    notes: string,
  ) => void;
}) {
  ...
  return (
    <div className="rounded-lg border border-border p-3 grid md:grid-cols-[120px_1fr_auto] gap-3">
      {m.signed_url ? (
        ...
        <div className="size-[120px] rounded-md bg-muted grid place-items-center text-[10px] text-muted-foreground">
          no preview
        </div>
      )}
      <div className="min-w-0 text-xs space-y-1">
        ...
        <div className="text-muted-foreground text-[10px]">
          face conf {Math.round(Number(m.face_confidence ?? 0))}% ·{" "}
        ...
        {m.review_status === "pending" && (
          <div className="mt-2 flex flex-wrap gap-2 items-center">
            <select
              value={cat}
              onChange={(e) => setCat(e.target.value as (typeof CATEGORIES)[number]["id"])}
            ...
            <input
              ...
              placeholder="Context notes (required)"
        ...
        {m.review_status !== "pending" && (
          <div className="text-[11px] mt-1">
            <span className="font-semibold uppercase">{m.review_status}</span>
            {m.threat_category ? ` · ${m.threat_category}` : ""}
            {m.context_notes ? ` — ${m.context_notes}` : ""}
          </div>
        )}
      </div>
      {m.review_status === "pending" && (
        <div className="flex flex-col gap-1 self-center">
          <button
            disabled={!notes}
            onClick={() => onDecide("authorized", undefined, notes || "authorized use")}
          ...
          <button
            disabled={!notes}
            onClick={() => onDecide("harmless", undefined, notes || "no harm")}
            className="text-[11px] px-2 py-1 rounded bg-secondary disabled:opacity-40"
          >
            Harmless
          </button>
          <button
            disabled={!notes}
            onClick={() => onDecide("threat_created", cat, notes)}
            className="text-[11px] px-2 py-1 rounded bg-destructive text-destructive-foreground disabled:opacity-40"
          >
            Create Threat
          </button>
          <button
            disabled={!notes}
            onClick={() => onDecide("dismissed", undefined, notes || "dismissed")}
            className="text-[11px] px-2 py-1 rounded bg-muted disabled:opacity-40"
          >
            Dismiss
          </button>
        </div>
