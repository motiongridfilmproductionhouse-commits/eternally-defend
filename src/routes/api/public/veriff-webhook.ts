  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const sig =
          request.headers.get("x-hmac-signature") ?? request.headers.get("x-signature") ?? "";
        ...
        const veriff_session_id: string | undefined =
          payload?.verification?.id ?? payload?.sessionId;
        const vendorData: string | undefined =
          payload?.verification?.vendorData ?? payload?.vendorData;
        const codeStatus: string | undefined =
          payload?.verification?.status ?? payload?.status ?? payload?.action;
        ...
        if (veriff_session_id) {
          await supabaseAdmin
            ...
            .eq("veriff_session_id", veriff_session_id);
        } else if (vendorData) {
          await supabaseAdmin
            .from("kyc_verifications")
            .update(patch as never)
