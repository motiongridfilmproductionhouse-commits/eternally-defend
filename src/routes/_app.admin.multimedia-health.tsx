...
import {
  getMultimediaHealth,
  testAllMultimediaProviders,
  testOneProvider,
} from "@/lib/mm/health.functions";
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full border ${s.cls}`}
    >
...
function HealthPage() {
  ...
  return (
    <div className="space-y-5">
      <PageCard
        title="MULTIMEDIA INTELLIGENCE HEALTH"
        sub="Live provider status, self-tests, quota and cost snapshot"
        actions={
      ...
      >
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
          <StatCard
            ...
            sub={`Limit ${limits.dailyAnalysisLimit}/day`}
            accent="oklch(0.65 0.18 260)"
          />
          <StatCard
            label="TODAY'S SPEND"
            value={`$${(usage.cost_cents / 100).toFixed(2)}`}
            sub={`Cap $${limits.monthlyCostLimitUsd}/month`}
          ...
          <StatCard
            label="MAX UPLOAD"
            value={`${limits.maxUploadMb} MB`}
            sub={`Video ≤ ${limits.maxVideoMinutes} min · retention ${limits.evidenceRetentionDays}d`}
              ...
              {providers.map((p) => {
                ...
                return (
                  <tr key={p.key} className="border-t border-border">
                    ...
                    <td className="p-2">
                      <StatusBadge status={s.currentAvailability ?? "unknown"} />
                    </td>
                    <td className="p-2 text-xs">
                    ...
                    <td className="p-2 text-xs text-muted-foreground">
                      {s.lastFailureAt ? new Date(s.lastFailureAt).toLocaleString() : "—"}
                    </td>
