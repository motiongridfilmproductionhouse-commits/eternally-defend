...
function AssetsPage() {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select("id,name,kind,source_url,active,metadata,storage_path,created_at")
  ...
  const remove = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ["protected_assets", userId] }),
  });
  const assets = query.data ?? [];
  const filtered = assets.filter((a) =>
    `${a.name} ${a.source_url ?? ""}`.toLowerCase().includes(q.toLowerCase()),
  );
  const monitoring = assets.filter((a) => a.metadata?.status === "Monitoring").length;
  const atRisk = assets.filter(
    (a) => Number(a.metadata?.reverse_search_match_count ?? 0) > 0,
  ).length;
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard
          label="PROTECTED"
          value={assets.length}
          sub="Fingerprint stored"
          accent="oklch(0.68 0.16 155)"
        />
        <StatCard
          label="MONITORING"
          value={monitoring}
          sub="Reverse searched"
          accent="oklch(0.75 0.16 70)"
        />
        <StatCard
          label="MATCHES FOUND"
          value={atRisk}
          sub="Assets with web matches"
            ...
            {result.reverse.pages.length === 0 ? (
              ...
              result.reverse.pages.map((p, i) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center gap-3 rounded-xl border border-border p-3 hover:bg-accent/40"
                >
      ...
      <PageCard
        ...
        sub="Uploaded, fingerprinted and reverse searched assets"
        actions={
          <div className="flex gap-2">
            <div className="relative">
              ...
              <input
                ...
                className="pl-9 pr-3 py-2 rounded-lg border border-border bg-card text-sm w-56"
              />
            </div>
            <button
      ...
      >
        {query.isLoading ? (
          ...
          <div className="space-y-2">
            {filtered.map((a) => (
              <div key={a.id} className="flex items-center gap-3 border-b border-border py-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {a.metadata?.reverse_search_match_count ?? 0} matches ·{" "}
      ...
      {open && (
        <div
          ...
          onClick={() => !add.isPending && setOpen(false)}
        >
          <div className="card-surface p-6 w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
            <div className="text-lg font-bold">Upload and protect an image</div>
            <div className="text-sm text-muted-foreground mb-4">
              The image will be stored, fingerprinted and reverse searched.
            </div>
            <div className="space-y-3">
              <label className="block text-xs font-semibold">
                Image (JPG, PNG or WebP; maximum 10 MB)
                <input
                  type="file"
            ...
            <div className="flex justify-end gap-2 mt-5">
              ...
              <button
                ...
                onClick={() => add.mutate()}
                className="px-4 py-2 bg-primary text-primary-foreground rounded-lg font-semibold inline-flex items-center gap-2 disabled:opacity-50"
              >
                {add.isPending ? (
                ...
                {add.isPending ? "Uploading & searching..." : "Upload & Reverse Search"}
              </button>
            </div>
          </div>
        </div>
