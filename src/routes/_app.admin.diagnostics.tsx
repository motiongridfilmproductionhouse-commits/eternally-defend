      ...
      >
        {isLoading ? (
          ...
          <div className="text-sm text-destructive p-4">
            Error loading diagnostics. Are you an admin?
          </div>
        ) : !status ? (
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <StatusCard
                ...
                description="Core database, auth, and backend logic"
              />
              <StatusCard
                title="Firecrawl"
                status={status.firecrawl}
                description="Web scraping and scanning capabilities"
              />
              <StatusCard
                title="YouTube API"
                status={status.youtube}
                description="Fetching video metadata and transcripts"
              />
              <StatusCard
                title="AWS Services"
                status={status.aws}
                description="Rekognition video processing and S3 storage"
              />
              <StatusCard
                title="Veriff KYC"
                status={status.veriff}
                description="Identity verification sessions and webhooks"
              />
              <StatusCard
                title="Google Cloud"
                status={status.googleCloud}
                description="Multimedia processing (Vision, Speech, etc)"
              />
              <StatusCard
                title="Eterna AI"
                status={status.ai}
                description="Gemini extraction and video classification"
              />
              <StatusCard
                title="Fact Checking"
                status={status.factChecking}
                description="Google Fact Check Tools API for claims"
              />
            </div>
          </div>
        )}
...
function StatusBadge({ status }: { status: string }) {
  if (status === "configured") {
    return (
      <Badge
        className="bg-emerald-500/15 text-emerald-600 border-emerald-500/30 whitespace-nowrap"
        variant="outline"
      >
    ...
    return (
      <Badge
        className="bg-amber-500/15 text-amber-700 border-amber-500/30 whitespace-nowrap"
        variant="outline"
      >
    ...
    return (
      <Badge
        className="bg-destructive/15 text-destructive border-destructive/30 whitespace-nowrap"
        variant="outline"
      >
