    ...
    <div className="space-y-5">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard
          ...
          sub="Clusters detected"
          accent="oklch(0.55 0.24 305)"
        />
        <StatCard
        ...
        <StatCard
          label="PEAK VELOCITY"
          value={topVelocity.toFixed(2)}
          sub="Sources per day"
          accent="oklch(0.72 0.14 70)"
        />
        <StatCard
          label="HIGH THREAT"
          value={String(clusters.filter((c) => c.threat_score >= 60).length)}
          sub="Score ≥ 60"
      ...
      <PageCard
        ...
        sub="Findings automatically grouped by claim, entity, source and content similarity"
        actions={
          <Button onClick={() => mut.mutate()} disabled={mut.isPending}>
            <Sparkles className="size-4 mr-2" />
            {mut.isPending ? "Clustering…" : "Recompute clusters"}
          </Button>
        }
      >
        {mut.data && (
          <div className="mb-3 text-xs text-emerald-600">
            Clustered {mut.data.linked} multimedia findings and {mut.data.importedChannelWatch}{" "}
            Channel Watch findings into {mut.data.clusters} narratives ({mut.data.created} new,{" "}
            {mut.data.updated} updated).
          </div>
        )}
        {q.isLoading ? (
          ...
          <div className="py-10 text-center text-sm text-muted-foreground">
            ...
            No eligible narratives clustered yet. Analyze review-level content in Channel Watch or
            Intelligence, then click <b>Recompute clusters</b>.
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="mt-2 grid grid-cols-4 gap-2 text-[11px]">
                    ...
                    <Metric
                      icon={TrendingUp}
                      label="Reach"
                      value={(c.combined_reach ?? 0).toLocaleString()}
                    />
                    <Metric
                      icon={TrendingUp}
                      label="Velocity"
                      value={`${c.narrative_velocity}/d`}
                    />
...
function ClusterDetail({ clusterId }: { clusterId: string }) {
  ...
  const removalMutation = useMutation({
    mutationFn: (videoId: string) => removalFn({ data: { videoId, clusterId } }),
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ["cluster", clusterId] });
    },
  });
  ...
  return (
    <div className="border border-border rounded-xl p-4 space-y-3">
      ...
      <dl className="grid grid-cols-2 gap-y-1 text-xs">
        ...
        <dd>{c.source_count}</dd>
        <dt className="text-muted-foreground">Combined reach</dt>
        <dd>{(c.combined_reach ?? 0).toLocaleString()}</dd>
        <dt className="text-muted-foreground">Dominant source</dt>
        <dd className="truncate">{c.dominant_source ?? "—"}</dd>
        <dt className="text-muted-foreground">First detected</dt>
        <dd>{new Date(c.first_detected_at).toLocaleString()}</dd>
        <dt className="text-muted-foreground">Latest detected</dt>
        <dd>{new Date(c.latest_detected_at).toLocaleString()}</dd>
      </dl>
      <div>
      ...
      <div>
        ...
        <ul className="text-xs space-y-1 max-h-80 overflow-auto">
          {channelWatchFindings.map((finding: any) => (
            <li key={`channel-${finding.id}`} className="border border-border rounded p-2">
              ...
              <div className="mt-1 font-medium">{finding.title ?? "Untitled video"}</div>
              ...
              <div className="mt-2 grid grid-cols-1 gap-1.5">
                ...
                <Button
                  ...
                  disabled={finding.review_status !== "approved" || removalMutation.isPending}
                  onClick={() => removalMutation.mutate(finding.id)}
                >
                  <Send className="mr-1 size-3" />
                  {removalMutation.isPending ? "Creating draft…" : "Send to Removal Center"}
                </Button>
          ...
          {findings.map((finding: any) => (
            <li key={`multimedia-${finding.id}`} className="border border-border rounded p-2">
              <Badge variant="outline" className="text-[9px]">
