...
function PartnerStatusPage() {
  ...
  const [data, setData] = useState<{
    ...
    profile: { partner_id: string } | null;
  } | null>(null);
      ...
      <main className="max-w-3xl mx-auto px-6 py-10 space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            {app.legal_company_name} · submitted {new Date(app.created_at).toLocaleDateString()}
          </p>
        </div>
        ...
        {agreement && (
          <Card className="border border-slate-200 shadow-sm p-6 rounded-xl">
            ...
            <div className="text-sm text-slate-600 mb-4">
              ...
              {agreement.eterna_signed_at && (
                <> · Countersigned {new Date(agreement.eterna_signed_at).toLocaleDateString()}</>
              )}
            </div>
            <div className="flex flex-wrap gap-3">
              <Button variant="outline" onClick={() => openAgreement("draft")}>
                View draft
              </Button>
        ...
        {partnerActive && (
          <Card className="border border-blue-200 bg-blue-50/50 p-6 rounded-xl">
            ...
            <p className="text-sm text-blue-800/80 mt-1">
              Partner ID: <span className="font-mono">{data.profile?.partner_id}</span>
            </p>
            <div className="mt-4">
              <Link to="/partner">
                <Button
...
function statusChip(status: string) {
  switch (status) {
    case "APPROVED":
      return {
        title: "Approved",
        desc: "Your partner account has been activated.",
        icon: CheckCircle2,
        bg: "bg-green-100",
        fg: "text-green-700",
      };
    ...
    case "INFO_REQUESTED":
      return {
        ...
        desc: "The Eterna team has asked for clarifications.",
        icon: AlertCircle,
        bg: "bg-amber-100",
        fg: "text-amber-700",
      };
    default:
      return {
        ...
        desc: "Our team is reviewing your application and documents.",
        icon: Clock,
        bg: "bg-blue-100",
        fg: "text-blue-700",
      };
