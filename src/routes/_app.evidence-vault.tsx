...
function EvidenceVault() {
  ...
  const uploadMut = useMutation({
    ...
    onSuccess: () => {
      toast.success("Uploaded to Evidence Vault");
      qc.invalidateQueries({ queryKey: ["evidence-vault"] });
    },
      ...
      <header className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Server-encrypted evidence stored in Amazon S3 · signed download links (5 min TTL)
          </p>
        </div>
      </header>
              ...
              {(q.data ?? []).map((r) => (
                <tr key={r.id} className="border-t border-border">
                  <td className="py-2 truncate max-w-[220px]">
                    {r.label ?? r.s3_key.split("/").pop()}
                  </td>
                  <td>{r.kind}</td>
                  <td>{r.bytes ?? "—"}</td>
                  <td className="font-mono truncate max-w-[180px]">
                    {r.sha256?.slice(0, 16) ?? "—"}
                  </td>
