...
function severityTone(sev?: string | null) {
  switch ((sev ?? "").toLowerCase()) {
    case "critical":
      return "oklch(0.6 0.24 25)";
    case "high":
      return "oklch(0.68 0.19 45)";
    case "medium":
      return "oklch(0.75 0.16 70)";
    case "low":
      return "oklch(0.68 0.16 155)";
    default:
      return "oklch(0.6 0.05 285)";
  }
}
...
function ThreatMonitoringPage() {
  ...
  return (
    <div className="space-y-5">
      <PageCard
        title="14-DAY THREAT TRENDS"
        sub={
          trends.data
            ? `${trends.data.totalHits} detections in the last 14 days`
            : "Detections across primary threat categories"
        }
      >
        <div className="h-64">
          {trends.isLoading ? (
            ...
            <ResponsiveContainer>
              <LineChart data={trends.data?.series ?? []}>
                ...
                <Tooltip
                  contentStyle={{ borderRadius: 12, border: "1px solid oklch(0.92 0.01 285)" }}
                />
      ...
      <PageCard
        ...
        actions={
          <input
            ...
            placeholder="Search..."
            className="text-sm px-3 py-2 rounded-lg border border-border bg-card w-56 focus:outline-none focus:ring-2 focus:ring-primary/20"
          />
        }
      >
            ...
            <tbody>
              ...
              {list.map((t: any) => (
                <tr key={t.id} className="border-b border-border/60 hover:bg-accent/30">
                  <td className="py-3 pr-4 font-medium max-w-[360px] truncate">
                    {cleanTitle(t.title, readableFromSlug(t.permalink ?? t.canonical_url))}
                  </td>
                  <td className="py-3 pr-4 text-muted-foreground">{t.risk_type ?? "—"}</td>
                  <td className="py-3 pr-4 text-muted-foreground">{t.source}</td>
                  <td className="py-3 pr-4">
                    <Pill color={severityTone(t.severity)}>{t.severity ?? "—"}</Pill>
                  </td>
                  ...
                  <td className="py-3 pr-4">
                    {t.permalink || t.canonical_url ? (
                      <a
                        href={t.permalink ?? t.canonical_url}
                        target="_blank"
