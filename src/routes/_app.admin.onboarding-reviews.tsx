...
function OnboardingReviewsPage() {
  ...
  const [actionType, setActionType] = useState<
    "approve" | "reject" | "request_info" | "suspend" | "revoke" | "renew" | null
  >(null);
  ...
  const handleAction = async () => {
    ...
    try {
      await decide({
        data: { authorization_id: selectedId, decision: actionType, notes: notes || undefined },
      });
                  ...
                  <tr key={item.id} className="border-b border-white/10 hover:bg-white/5">
                    <td className="px-6 py-4">
                      <div className="text-white font-medium">
                        {item.client_profiles?.display_name || "Unknown"}
                      </div>
                      <div className="text-xs text-white/50 font-mono">
                        {item.client_profiles?.client_id}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-2 py-1 rounded text-[10px] uppercase font-semibold ${
                          item.status === "UNDER_ADMIN_REVIEW"
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="bg-white/5 rounded-lg p-4 border border-white/10">
                  ...
                  <div
                    className={`text-sm font-semibold mt-1 ${detail.kyc?.verification_status === "APPROVED" ? "text-emerald-400" : "text-orange-400"}`}
                  >
                ...
                <div className="bg-white/5 rounded-lg p-4 border border-white/10">
                  ...
                  <div
                    className={`text-sm font-semibold mt-1 ${detail.face?.status === "FACE_VERIFIED" ? "text-emerald-400" : "text-orange-400"}`}
                  >
                ...
                <div className="bg-white/5 rounded-lg p-4 border border-white/10">
                  ...
                  <div
                    className={`text-sm font-semibold mt-1 ${detail.assets?.some((a: any) => a.verification_status === "VERIFIED") ? "text-emerald-400" : "text-orange-400"}`}
                  >
                    {detail.assets?.some((a: any) => a.verification_status === "VERIFIED")
                      ? "VERIFIED"
                      : "MISSING"}
                  </div>
                </div>
              ...
              <div className="bg-white/5 border border-white/10 rounded-lg p-4">
                <h3 className="text-sm font-semibold text-white/80 border-b border-white/10 pb-2 mb-3">
                  Client Profile
                </h3>
                <div className="grid grid-cols-2 gap-y-2 text-sm">
                  <div className="text-white/50">Full Name</div>
                  <div className="text-white">
                    {detail.profile?.full_name || detail.profile?.display_name || "-"}
                  </div>
                  <div className="text-white/50">Company</div>
                  <div className="text-white">{detail.profile?.company_name || "-"}</div>
                  <div className="text-white/50">Email</div>
                  <div className="text-white">{detail.profile?.email || "-"}</div>
                  <div className="text-white/50">Phone</div>
                  <div className="text-white">{detail.profile?.phone || "-"}</div>
                </div>
              </div>
              ...
              <div className="bg-white/5 border border-white/10 rounded-lg p-4">
                ...
                {(() => {
                  const signedDoc = detail.documents?.find(
                    (d: any) => d.kind === "signed" && d.version === detail.auth?.version,
                  );
                  const sig = detail.signatures?.find(
                    (s: any) => s.status === "SIGNED" && s.version === detail.auth?.version,
                  );
                  return signedDoc && sig ? (
                    <div className="space-y-3 text-sm">
                      <div className="flex justify-between items-center bg-blue-500/10 p-3 rounded-lg border border-blue-500/20">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-blue-300">
                            Signed Authorization Letter v{detail.auth.version}
                          </span>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-y-1 text-xs px-2">
                        <div className="text-white/50">Signed By</div>
                        <div className="text-white">
                          {sig.typed_name} ({sig.role_title || "Owner"})
                        </div>
                        <div className="text-white/50">Timestamp</div>
                        <div className="text-white">
                          {sig.signed_at ? new Date(sig.signed_at).toLocaleString() : "Unknown"}
                        </div>
                        <div className="text-white/50">IP Address</div>
                        <div className="text-white font-mono">{sig.ip_address}</div>
                        <div className="text-white/50">SHA-256 Hash</div>
                        <div
                          className="text-white font-mono truncate max-w-[200px]"
                    ...
                    <div className="text-sm text-white/50">
                      No valid signed document found for current version.
                    </div>
                  );
                })()}
              </div>
                    ...
                    {detail.reviews.map((r: any) => (
                      <div key={r.id} className="text-xs bg-black/20 p-2 rounded">
                        <div className="flex justify-between text-white/50 mb-1">
                          <span className="uppercase font-semibold text-white/70">
              ...
              {actionType ? (
                <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4 space-y-4">
                  ...
                  {actionType !== "approve" && (
                    <div className="space-y-2">
                      <label className="text-xs text-white/60">
                        Reason / Notes to Client (Required)
                      </label>
                      <Textarea
                        value={notes}
                        ...
                        placeholder={
                          actionType === "request_info"
                            ? "e.g. Please correct your asset documentation."
                  ...
                  <div className="flex gap-3 justify-end">
                    ...
                    <Button
                      ...
                      disabled={busy || (actionType !== "approve" && !notes.trim())}
                      className="bg-blue-600 hover:bg-blue-500 text-white"
                    >
                      {busy ? <Loader2 className="size-4 animate-spin mr-2" /> : null} Submit
                      Decision
                    </Button>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-3 pt-2">
                  ...
                  >
                    <AlertTriangle className="size-4 mr-2" /> Request Info
                  </Button>
                  <Button
                    variant="outline"
                  ...
                  <Button
                    ...
                    className="border-red-500/50 text-red-400 hover:bg-red-500/10"
                  >
                    <ShieldAlert className="size-4 mr-2" /> Suspend
                  </Button>
                  <Button
                    variant="outline"
                    ...
                    className="border-red-500/50 text-red-400 hover:bg-red-500/10"
                  >
                    <History className="size-4 mr-2" /> Revoke
                  </Button>
                </div>
              )}
            </div>
