  ...
  return (
    <div className="space-y-5">
      <PageCard
        ...
        sub="Enable real Google Cloud providers by setting environment variables. No code changes required."
        actions={
          <Button onClick={() => testAll.mutate()} disabled={testAll.isPending}>
            <PlayCircle className="size-4 mr-2" />
            {testAll.isPending ? "Validating…" : "Validate credentials"}
          </Button>
        }
      >
        {!cfg ? (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 text-[11px] text-muted-foreground">
                        Feature flag <code className="bg-muted px-1 rounded">{p.flag}</code> ={" "}
                        <code className="bg-muted px-1 rounded">{mode}</code>
                      </div>
                    </div>
                  </div>
      ...
      <PageCard
        ...
        sub="Scheduled cleanup of temporary media, transcripts and processing artefacts. Audit logs are always retained."
        actions={
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => cleanupDry.mutate()}
              disabled={cleanupDry.isPending}
            >
              Preview cleanup
            </Button>
            <Button onClick={() => cleanupRun.mutate()} disabled={cleanupRun.isPending}>
              <Trash2 className="size-4 mr-2" />
              {cleanupRun.isPending ? "Running…" : "Run cleanup"}
            </Button>
          </div>
        }
      >
        ...
        {cleanupDry.data && (
          <div className="text-xs bg-muted/40 rounded p-2">
            Preview: {cleanupDry.data.jobsMatched} job(s) eligible. Retention buckets:{" "}
            {JSON.stringify(cleanupDry.data.summary)}.
          </div>
        )}
        ...
        <div className="mt-3 text-[11px] text-muted-foreground space-y-0.5">
          <div>
            <b>Immediate:</b> purged when job finishes.
          </div>
          <div>
            <b>7 / 30 days:</b> temp media &amp; transcripts cleaned after window.
          </div>
          <div>
            <b>Case closure:</b> retained until linked case is closed.
          </div>
          <div>
            <b>Legal hold:</b> never cleaned.
          </div>
          <div>
            Always retained: job records, audit history, extracted claims, fact-check matches,
            narrative clusters.
          </div>
        </div>
      </PageCard>
